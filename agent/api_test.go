...
func TestAPIJobCreateUpdate(t *testing.T) {
	...
	jsonStr := []byte(`{"name": "test_job","disabled": true}`)

	resp, err := http.Post("http://localhost:13000/v1/jobs", "encoding/json", bytes.NewBuffer(jsonStr))
	if err != nil {
		t.Fatal(err)
	}
	...
	jsonStr1 := []byte(`{"name": "test_job","disabled": false}`)
	resp, err = http.Post("http://localhost:13000/v1/jobs", "encoding/json", bytes.NewBuffer(jsonStr1))
	if err != nil {
		t.Fatal(err)
	}
	...
	assert.NotEqual(t, origJob.Type, overwriteJob.Type)
	assert.Equal(t, "mysql", overwriteJob.Driver)
