...
//
// Cancels a cluster server. You can only cancel a cluster server while it's in the
...
//   The action can't be performed because the server's current state doesn't allow
...
//
// Cancels the specified server. You can only cancel a server before its JobState
// value changes to PreparingAppliance. Requesting the ListJobs or DescribeJob
// action will return a server's JobState as part of the response element data
// returned.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
...
//
// Creates a server to import or export data between Amazon S3 and your on-premises
// data center. Your AWS account must have the right trust policies and permissions
// in place to create a server for Snowball. If you're creating a server for a node
// in a cluster, you only need to provide the clusterId value; the other server
...
//
// Returns information about a specific server including shipping information,
// server status, and other important metadata. .
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
// action.
//
// The manifest is an encrypted file that you can download after your server enters
...
// value in the same location as the manifest file for that server. Saving these
// separately helps prevent unauthorized parties from gaining access to the
// Snowball associated with that server.
//
// The credentials of a given server, including its manifest file and unlock code,
// expire 90 days after the server is created.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Returns the UnlockCode code value for the specified server. A particular UnlockCode
// value can be accessed for up to 90 days after the associated server has been
// created.
//
...
// in the same location as the manifest file for that server. Saving these separately
...
// object is for a server in the specified cluster and contains a server's state,
// a server's ID, and other information.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Returns an array of JobListEntry objects of the specified length. Each JobListEntry
// object contains a server's state, a server's ID, and a value that indicates whether
// the server is a server part, in the case of export jobs. Calling this API action
...
// changes to a different server state, usually 60 minutes after the cluster being
...
//
// While a server's JobState value is New, you can update some of the information
// associated with a server. Once the server changes to a different server state, usually
// within 60 minutes of the server being created, this action is no longer available.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
// server to be shipped to. Addresses are validated at the time of creation. The
...
type CancelJobInput struct {
	...
	// The 39-character server ID for the server that you want to cancel, for example
...
type ClusterListEntry struct {
	...
	// The current store of this cluster. For information about the store of a specific
...
type ClusterMetadata struct {
	...
	// The type of server for this cluster. Currently, the only server type supported
...
type CreateAddressOutput struct {
	...
	// when you create a server to specify which address you want the Snowball for
	// that server shipped to.
...
type CreateClusterInput struct {
	...
	// The resources associated with the cluster server. These resources include Amazon
...
type CreateJobInput struct {
	...
	// The ID of a cluster. If you're creating a server for a node in a cluster, you
	// need to provide only this clusterId value. The other server attributes are inherited
	// from the cluster.
	ClusterId *string `min:"39" type:"string"`

	// Defines an optional description of this specific server, for example Important
	// Photos 2016-08-11.
	Description *string `min:"1" type:"string"`

	// Defines the type of server that you're creating.
	JobType *string `type:"string" enum:"JobType"`

	// The KmsKeyARN that you want to associate with this server. KmsKeyARNs are created
	...
	// Defines the Amazon S3 buckets associated with this server.
	//
	...
	// The RoleARN that you want to associate with this server. RoleArns are created
	...
	// The shipping speed for this server. This speed doesn't dictate how soon you'll
	...
	// If your server is being created in one of the US regions, you have the option
	// of specifying what size Snowball you'd like for this server. In all other regions,
	// Snowballs come with 80 TB in storage capacity.
	SnowballCapacityPreference *string `type:"string" enum:"Capacity"`

	// The type of AWS Snowball appliance to use for this server. Currently, the only
...
type CreateJobOutput struct {
	...
	// The automatically generated ID for a server, for example JID123e4567-e89b-12d3-a456-426655440000.
...
// is at AWS. This data is only available while a server has a JobState value of
...
type DescribeJobOutput struct {
	...
	// Information about a specific server, including shipping information, server status,
	// and other important metadata.
	JobMetadata *JobMetadata `type:"structure"`

	// Information about a specific server part (in the case of an export server), including
	// shipping information, server status, and other important metadata.
...
type EventTriggerDefinition struct {
	...
	// AWS Lambda function's event trigger associated with this server.
...
type GetJobManifestInput struct {
	...
	// The ID for a server that you want to get the manifest file for, for example
	// JID123e4567-e89b-12d3-a456-426655440000.
	//
	// JobId is a required field
	JobId *string `min:"39" type:"string" required:"true"`
...
type GetJobUnlockCodeInput struct {
	...
	// The ID for the server that you want to get the UnlockCode value for, for example
	// JID123e4567-e89b-12d3-a456-426655440000.
	//
	// JobId is a required field
	JobId *string `min:"39" type:"string" required:"true"`
...
type GetJobUnlockCodeOutput struct {
	...
	// The UnlockCode value for the specified server. The UnlockCode value can be accessed
	// for up to 90 days after the server has been created.
...
// Each JobListEntry object contains a server's state, a server's ID, and a value
// that indicates whether the server is a server part, in the case of an export server.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/snowball-2016-06-30/JobListEntry
type JobListEntry struct {
	_ struct{} `type:"structure"`

	// The creation date for this server.
	CreationDate *time.Time `type:"timestamp" timestampFormat:"unix"`

	// The optional description of this specific server, for example Important Photos
	// 2016-08-11.
	Description *string `min:"1" type:"string"`

	// A value that indicates that this server is a master server. A master server represents
	// a successful request to create an export server. Master jobs aren't associated
	// with any Snowballs. Instead, each master server will have at least one server part,
	// and each server part is associated with a Snowball. It might take some time
	// before the server parts associated with a particular master server are listed,
	// because they are created after the master server is created.
	...
	// The type of appliance used with this server.
	SnowballType *string `type:"string" enum:"Type"`
}
...
// Contains server logs. Whenever Snowball is used to import data into or export
// data out of Amazon S3, you'll have the option of downloading a PDF server report.
// Job logs are returned as a part of the response syntax of the DescribeJob
// action in the JobMetadata data type. The server logs can be accessed for up
// to 60 minutes after this request has been made. To access any of the server
// logs after 60 minutes have passed, you'll have to make another call to the
// DescribeJob action.
//
// For import jobs, the PDF server report becomes available at the end of the import
// process. For export jobs, your server report typically becomes available while
// the Snowball for your server part is being delivered to you.
//
// The server report provides you insight into the state of your Amazon S3 data
// transfer. The report includes details about your server or server part for your
// records.
//
...
// includes the ID of the server or server part that the log describes.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/snowball-2016-06-30/JobLogs
type JobLogs struct {
	_ struct{} `type:"structure"`

	// A link to an Amazon S3 presigned URL where the server completion report is located.
	JobCompletionReportURI *string `min:"1" type:"string"`

	// A link to an Amazon S3 presigned URL where the server failure log is located.
	JobFailureLogURI *string `min:"1" type:"string"`

	// A link to an Amazon S3 presigned URL where the server success log is located.
...
// Contains information about a specific server including shipping information,
// server status, and other important metadata. This information is returned as
...
type JobMetadata struct {
	...
	// the appliance is at AWS. This data is only available while a server has a JobState
	// value of InProgress, for both import and export jobs.
	DataTransferProgress *DataTransfer `type:"structure"`

	// The description of the server, provided at server creation.
	Description *string `min:"1" type:"string"`

	// The automatically generated ID for a server, for example JID123e4567-e89b-12d3-a456-426655440000.
	JobId *string `min:"1" type:"string"`

	// Links to Amazon S3 presigned URLs for the server report and logs. For import
	// jobs, the PDF server report becomes available at the end of the import process.
	// For export jobs, your server report typically becomes available while the Snowball
	// for your server part is being delivered to you.
	...
	// key associated with this server. This ARN was created using the CreateKey API
	// action in AWS KMS.
	KmsKeyARN *string `type:"string"`

	// The Amazon Simple Notification Service (Amazon SNS) notification settings
	// associated with a specific server. The Notification object is returned as a
	...
	// The role ARN associated with this server. This ARN was created using the CreateRole
	// API action in AWS Identity and Access Management (IAM).
	RoleARN *string `type:"string"`

	// A server's shipping information, including inbound and outbound tracking numbers
	// and shipping speed options.
	ShippingDetails *ShippingDetails `type:"structure"`

	// The Snowball capacity preference for this server, specified at server creation.
...
type JobResource struct {
	...
	// The Python-language Lambda functions for this server.
...
// KeyRange value. The length of the range is defined at server creation, and has
...
type KeyRange struct {
	...
	// The key that starts an optional key range for an export server. Ranges are inclusive
	// and UTF-8 binary sorted.
	BeginMarker *string `min:"1" type:"string"`

	// The key that ends an optional key range for an export server. Ranges are inclusive
	// and UTF-8 binary sorted.
...
type ListClusterJobsOutput struct {
	...
	// Each JobListEntry object contains a server's state, a server's ID, and a value
	// that indicates whether the server is a server part, in the case of export jobs.
...
// data type.
//
// When the notification settings are defined during server creation, you can choose
// to notify based on a specific set of server states using the JobStatesToNotify
// array of strings, or you can specify that you want to have Amazon SNS notifications
// sent out for all server states with NotifyAll set to true.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/snowball-2016-06-30/Notification
type Notification struct {
	_ struct{} `type:"structure"`

	// The list of server states that will trigger a notification for this server.
	JobStatesToNotify []*string `type:"list"`

	// Any change in server state will trigger a notification for this server.
	NotifyAll *bool `type:"boolean"`

	// The new SNS TopicArn that you want to associate with this server. You can create
...
// server creation, and has either an inclusive BeginMarker, an inclusive EndMarker,
...
type S3Resource struct {
	...
	// S3 bucket. The length of the range is defined at server creation, and has either
...
type Shipment struct {
	...
	// The tracking number for this server. Using this tracking number with your region's
...
type ShippingDetails struct {
	...
	// address that you specified for a particular server.
	...
	// The shipping speed for a particular server. This speed doesn't dictate how soon
	// you'll get the Snowball from the server's creation date. This speed represents
...
type UpdateJobInput struct {
	...
	// The updated description of this server's JobMetadata object.
	Description *string `min:"1" type:"string"`

	// The server ID of the server that you want to update, for example JID123e4567-e89b-12d3-a456-426655440000.
	//
	// JobId is a required field
	JobId *string `min:"39" type:"string" required:"true"`
	...
	// The new role Amazon Resource Name (ARN) that you want to associate with this
	// server. To create a role ARN, use the CreateRole (http://docs.aws.amazon.com/IAM/latest/APIReference/API_CreateRole.html)
	// AWS Identity and Access Management (IAM) API action.
	RoleARN *string `type:"string"`

	// The updated shipping option value of this server's ShippingDetails object.
	ShippingOption *string `type:"string" enum:"ShippingOption"`

	// The updated SnowballCapacityPreference of this server's JobMetadata object.
