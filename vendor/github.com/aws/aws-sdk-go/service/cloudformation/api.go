...
//
// You can cancel only stacks that are in the UPDATE_IN_PROGRESS store.
//
...
//
// For a specified stack that is in the UPDATE_ROLLBACK_FAILED store, continues
// rolling it back to the UPDATE_ROLLBACK_COMPLETE store. Depending on the cause
...
// stack to a working store (the UPDATE_ROLLBACK_COMPLETE store), and then try
// to update the stack again.
//
// A stack goes into the UPDATE_ROLLBACK_FAILED store when AWS CloudFormation
...
type ChangeSetSummary struct {
	...
	// a change set might be in an UNAVAILABLE store because AWS CloudFormation
	// is still creating it or in an OBSOLETE store because the stack was already
	// updated.
	...
	// The store of the change set, such as CREATE_IN_PROGRESS, CREATE_COMPLETE,
	// or FAILED.
	Status *string `type:"string" enum:"ChangeSetStatus"`

	// A description of the change set's status. For example, if your change set
	// is in the FAILED store, AWS CloudFormation shows the error message.
...
type ContinueUpdateRollbackInput struct {
	...
	// that are in the UPDATE_FAILED store because a rollback failed. You can't
	// specify resources that are in the UPDATE_FAILED store for other reasons,
	...
	// After the rollback is complete, the store of the skipped resources will be
	// inconsistent with the store of the resources in the stack template. Before
...
type CreateChangeSetInput struct {
	...
	// store until you execute the change set.
	//
...
type DeleteStackInput struct {
	...
	// For stacks in the DELETE_FAILED store, a list of resource logical IDs that
