...
// in the Locked store. If the vault lock is in the Locked store when this operation
...
// lock from the InProgress store to the Locked store, which causes the vault
// lock policy to become unchangeable. A vault lock is put into the InProgress
// store by calling InitiateVaultLock. You can obtain the store of the vault
...
//
//    * If the archive retrieval server is actively preparing the data for download
//    when Amazon Glacier receives the delete archive request, the archival
//    retrieval operation might fail.
//
//    * If the archive retrieval server has successfully prepared the archive for
...
//
// This operation returns information about a server you previously initiated,
// including the server initiation date, the user who initiated the server, the server
// status code/message and the Amazon SNS topic to notify after Amazon Glacier
// completes the server. For more information about initiating a server, see InitiateJob.
//
// This operation enables you to check the status of your server. However, it is
// strongly recommended that you set up an Amazon SNS topic and specify it in
// your initiate server request so that Amazon Glacier can notify the topic after
// it completes the server.
//
// A server ID will not expire for at least 24 hours after Amazon Glacier completes
// the server.
//
// An AWS account has full permission to perform all operations (actions). However,
// AWS Identity and Access Management (IAM) users don't have any permissions
...
//
// This operation downloads the output of the server you initiated using InitiateJob.
// Depending on the server type you specified when you initiated the server, the output
// will be either the content of an archive or a vault inventory.
//
// You can download all the server output or download a portion of the output by
// specifying a byte range. In the case of an archive retrieval server, depending
...
//
// A server ID will not expire for at least 24 hours after Amazon Glacier completes
// the server. That a byte range. For both archive and inventory retrieval jobs,
...
//
// In the case of an archive retrieval server, depending on the byte range you
...
// what you expected.
//
// A server ID does not expire for at least 24 hours after Amazon Glacier completes
// the server. That is, you can download the server output within the 24 hours period
// after Amazon Glacier completes the server.
//
// An AWS account has full permission to perform all operations (actions). However,
// AWS Identity and Access Management (IAM) users don't have any permissions
// by default. You must grant them explicit permission to perform specific actions.
...
//
// This operation initiates a server of the specified type. In this release, you
// can initiate a server to retrieve either an archive or a vault inventory (a
// list of archives in a vault).
//
// Retrieving data from Amazon Glacier is a two-step process:
//
// Initiate a retrieval server.
//
// A data retrieval policy can cause your initiate retrieval server request to
...
//
// After the server completes, download the bytes.
//
// The retrieval request is executed asynchronously. When you initiate a retrieval
// server, Amazon Glacier creates a server and returns a server ID in the response. When
// Amazon Glacier completes the server, you can get the server output (archive or
// inventory data). For information about getting server output, see GetJobOutput
// operation.
//
// The server must complete before you can get its output. To determine when a
// server is complete, you have the following options:
//
...
//    after the server is completed. You can specify an SNS topic per server request.
//    The notification is sent only after Amazon Glacier completes the server.
//    In addition to specifying an SNS topic per server request, you can configure
//    vault notifications for a vault so that server notifications are always sent.
//    For more information, see SetVaultNotifications.
//
//    * Get server details You can make a DescribeJob request to obtain server status
//    information while a server is in progress. However, it is more efficient
//    to use an Amazon SNS notification to determine when a server is complete.
//
...
// vault and also specify an SNS topic in your initiate server request, Amazon
...
// hours. When you initiate a server for a vault inventory, Amazon Glacier returns
// the last inventory for the vault. The inventory data you get might be up
// to a day or two days old. Also, the initiate inventory server might take some
...
// parameter in the InitiateJob request. The inventory server output will contain
// inventory items up to the specified Limit. If there are more inventory items
// available, the result is paginated. After a server is complete you can use the
...
// they have completed enables you to get a server output in the event you miss
// the server completion notification or your first attempt to download it fails.
// For example, suppose you start an archive retrieval server to download an archive.
// After the server completes, you start to download the archive but encounter
...
// you first initiate a server, and after the server completes, you download the data.
...
// of these jobs sorted by server initiation time.
//
...
// To return a list of jobs that begins at a specific server, set the marker request
// parameter to the Marker value for that server that you obtained from a previous
// List Jobs request.
//
...
//
//    * ArchiveRetrievalCompleted This event occurs when a server that was initiated
//    for an archive retrieval is completed (InitiateJob). The status of the
//    completed server can be "Succeeded" or "Failed". The notification sent to
//    the SNS topic is the same output as returned from DescribeJob.
//
//    * InventoryRetrievalCompleted This event occurs when a server that was initiated
...
// Provides options for retrieving a server description.
type DescribeJobInput struct {
	...
	// The ID of the server to describe.
	//
...
// Provides options for downloading output of an Amazon Glacier server.
type GetJobOutputInput struct {
	...
	// The server ID whose data is downloaded.
	//
	// JobId is a required field
	JobId *string `location:"uri" locationName:"jobId" type:"string" required:"true"`
	...
	//
	// If the server output is large, then you can use a range to retrieve a portion
	// of the output. This allows you to download the entire output in smaller chunks
	// of bytes. For example, suppose you have 1 GB of server output you want to download
	...
	//
	// After downloading all the parts of the server output, you have a list of eight
	// checksum values. Compute the tree hash of these values to find the checksum
	// of the entire output. Using the DescribeJob API, obtain server information of
	// the server that provided you the output. The response includes the checksum
...
type GetJobOutputOutput struct {
	...
	// The server data, either archive data or inventory data.
	Body io.ReadCloser `locationName:"body" type:"blob"`

	// The checksum of the data in the response. This header is returned only when
	// retrieving the output for an archive retrieval server. Furthermore, this header
	...
	// The Content-Type depends on whether the server output is an archive or a vault
	// inventory. For archive data, the Content-Type is application/octet-stream.
	// For vault inventory, if you requested CSV format when you initiated the server,
	...
	// The HTTP response code for a server output request. The value depends on whether
...
// Provides options for initiating an Amazon Glacier server.
type InitiateJobInput struct {
	...
	// Provides options for specifying server information.
...
type InitiateJobOutput struct {
	...
	JobId *string `location:"header" locationName:"x-amz-server-id" type:"string"`

	// The relative URI path of the server.
	Location *string `location:"header" locationName:"Location" type:"string"`
}
...
// Describes the options for a range inventory retrieval server.
type InventoryRetrievalJobDescription struct {
	...
	// request when initiating a server to retrieve a vault inventory. Valid values
	// are CSV and JSON.
	Format *string `type:"string"`

	// The maximum number of inventory items returned per vault inventory retrieval
	// request. This limit is set when initiating the server with the a InitiateJob
	// request.
...
// Provides options for specifying a range inventory retrieval server.
...
// Describes an Amazon Glacier server.
type JobDescription struct {
	_ struct{} `type:"structure"`

	// The server type. It is either ArchiveRetrieval or InventoryRetrieval.
	Action *string `type:"string" enum:"ActionCode"`

	// For an ArchiveRetrieval server, this is the archive ID requested for download.
	...
	// For an ArchiveRetrieval server, this is the size in bytes of the archive being
	// requested for download. For the InventoryRetrieval server, the value is null.
	ArchiveSizeInBytes *int64 `type:"long"`

	// The server status. When a server is completed, you get the server's output.
	Completed *bool `type:"boolean"`

	// The UTC time that the archive retrieval request completed. While the server
	// is in progress, the value will be null.
	CompletionDate *string `type:"string"`

	// The UTC date when the server was created. A string representation of ISO 8601
	...
	// For an InventoryRetrieval server, this is the size in bytes of the inventory
	// requested for download. For the ArchiveRetrieval server, the value is null.
	InventorySizeInBytes *int64 `type:"long"`

	// The server description you provided when you initiated the server.
	JobDescription *string `type:"string"`

	// An opaque string that identifies an Amazon Glacier server.
	...
	// For an ArchiveRetrieval server, it is the checksum of the archive. Otherwise,
	// the value is null.
	//
	...
	//    and the server status is InProgress.
	//
	//    * Inventory jobs.
	...
	// A friendly message that describes the server status.
...
// Provides options for defining a server.
type JobParameters struct {
	...
	// request parameter for an inventory retrieval server request.
	ArchiveId *string `type:"string"`

	// The optional description for the server. The description must be less than or
	...
	// When initiating a server to retrieve a vault inventory, you can optionally add
	// this parameter to your request to specify the output format. If you are initiating
	// an inventory server and do not specify a Format field, JSON is the default format.
	...
	// a 400 response.
	//
	// An error occurs if you specify this field for an inventory retrieval server
	// request.
	RetrievalByteRange *string `type:"string"`

	// The Amazon SNS topic ARN to which Amazon Glacier sends a notification when
	// the server is completed and the output is ready for you to download. The specified
	...
	// The server type. You can initiate a server to retrieve an archive or get an inventory
...
// Provides options for retrieving a server list for an Amazon Glacier vault.
type ListJobsInput struct {
	...
	// An opaque string used for pagination. This value specifies the server at which
	...
	// The type of server status to return. You can specify the following values: InProgress,
	// Succeeded, or Failed.
...
type ListJobsOutput struct {
	...
	// A list of server objects. Each server object contains metadata describing the server.
	JobList []*JobDescription `type:"list"`

	// An opaque string used for pagination that specifies the server at which the
