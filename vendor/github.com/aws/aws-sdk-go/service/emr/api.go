...
// are allowed in each server flow.
//
...
// instead.
//
// DescribeJobFlows returns a list of server flows that match all of the supplied
// parameters. The parameters can include a list of server flow IDs, server flow states,
// and restrictions on server flow creation date and time.
//
// Regardless of supplied parameters, only server flows created within the last
// two months are returned.
//
// If no parameters are supplied, then server flows matching either of the following
...
//
// Amazon EMR can return a maximum of 512 server flow descriptions.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// RunJobFlow creates and starts running a new cluster (server flow). The cluster
...
// of the server flow to store results in Amazon S3. If the JobFlowInstancesConfigKeepJobFlowAliveWhenNoSteps
...
// by API call, user intervention, or in the event of a server flow error.
//
// A maximum of 256 steps are allowed in each server flow.
//
// If your cluster is long-running (such as a Hive data warehouse) or complex,
// you may require more than 256 steps to process your data. You can bypass
...
//
// SetTerminationProtection locks a cluster (server flow) so the EC2 instances
// in the cluster cannot be terminated by user intervention, an API call, or
// in the event of a server-flow error. The cluster still terminates upon successful
// completion of the server flow. Calling SetTerminationProtection on a cluster
...
// to true, you must first unlock the server flow by a subsequent call to SetTerminationProtection
...
// account can access the specified clusters (server flows). This action works
...
//
// TerminateJobFlows shuts a list of clusters (server flows) down. When a server flow
...
type AddInstanceGroupsOutput struct {
	...
	// The server flow ID in which the instance groups are added.
...
type AddJobFlowStepsInput struct {
	...
	// A string that uniquely identifies the server flow. This identifier is returned
	// by RunJobFlow and can also be obtained from ListClusters.
	//
	// JobFlowId is a required field
	JobFlowId *string `type:"string" required:"true"`

	// A list of StepConfig to be executed by the server flow.
	//
...
type AddJobFlowStepsOutput struct {
	...
	// The identifiers of the list of steps added to the server flow.
...
// Reports the configuration of a bootstrap action in a cluster (server flow).
...
type DescribeJobFlowsInput struct {
	...
	// Return only server flows created after this date and time.
	CreatedAfter *time.Time `type:"timestamp" timestampFormat:"unix"`

	// Return only server flows created before this date and time.
	CreatedBefore *time.Time `type:"timestamp" timestampFormat:"unix"`

	// Return only server flows whose server flow ID is contained in this list.
	JobFlowIds []*string `type:"list"`

	// Return only server flows whose state is contained in this list.
...
type DescribeJobFlowsOutput struct {
	...
	// A list of server flows matching the parameters supplied.
...
// A server flow step consisting of a JAR file whose main function will be executed.
// The main function submits a server for Hadoop to execute and waits for the server
// to finish or fail.
...
type JobFlowDetail struct {
	...
	// A list of the bootstrap actions run by the server flow.
	BootstrapActions []*BootstrapActionDetail `type:"list"`

	// Describes the execution status of the server flow.
	//
	// ExecutionStatusDetail is a required field
	ExecutionStatusDetail *JobFlowExecutionStatusDetail `type:"structure" required:"true"`

	// Describes the Amazon EC2 instances of the server flow.
	//
	// Instances is a required field
	Instances *JobFlowInstancesDetail `type:"structure" required:"true"`

	// The server flow identifier.
	//
	// JobFlowId is a required field
	JobFlowId *string `type:"string" required:"true"`

	// The IAM role that was specified when the server flow was launched. The EC2 instances
	// of the server flow assume this role.
	JobFlowRole *string `type:"string"`

	// The location in Amazon S3 where log files for the server are stored.
	LogUri *string `type:"string"`

	// The name of the server flow.
	//
	// Name is a required field
	Name *string `type:"string" required:"true"`
	...
	// A list of steps run by the server flow.
	Steps []*StepDetail `type:"list"`

	// A list of strings set by third party software when the server flow is launched.
	// If you are not using third party software to manage the server flow this value
	// is empty.
...
// Describes the status of the cluster (server flow).
// Please also see https://docs.aws.amazon.com/goto/WebAPI/elasticmapreduce-2009-03-31/JobFlowExecutionStatusDetail
type JobFlowExecutionStatusDetail struct {
	_ struct{} `type:"structure"`

	// The creation date and time of the server flow.
	//
	// CreationDateTime is a required field
	CreationDateTime *time.Time `type:"timestamp" timestampFormat:"unix" required:"true"`

	// The completion date and time of the server flow.
	EndDateTime *time.Time `type:"timestamp" timestampFormat:"unix"`

	// Description of the server flow last changed state.
	LastStateChangeReason *string `type:"string"`

	// The date and time when the server flow was ready to start running bootstrap
	// actions.
	ReadyDateTime *time.Time `type:"timestamp" timestampFormat:"unix"`

	// The start date and time of the server flow.
	StartDateTime *time.Time `type:"timestamp" timestampFormat:"unix"`

	// The state of the server flow.
	//
	// State is a required field
...
// A description of the Amazon EC2 instance on which the cluster (server flow)
...
// Specify the type of Amazon EC2 instances that the cluster (server flow) runs
// on.
...
type JobFlowInstancesDetail struct {
	...
	// termination by API calls, user intervention, or in the event of a server-flow
	// error.
	TerminationProtected *bool `type:"boolean"`
}
...
// The Amazon EC2 Availability Zone configuration of the cluster (server flow).
...
type RunJobFlowInput struct {
	...
	// EC2 instances in the server flow. The following values are valid:
	//
	...
	// The location in Amazon S3 to write the log files of the server flow. If a value
	...
	// use Applications.
	//
	// A list of strings that indicates third-party software to use with the server
	...
	//
	//    * "mapr" with the user arguments specifying "--edition,m3" or "--edition,m5"
	//    - launch the server flow using MapR M3 or M5 Edition respectively.
	//
	...
	// Currently supported values are:
	//
	//    * "mapr-m3" - launch the server flow using MapR M3 Edition.
	//
	//    * "mapr-m5" - launch the server flow using MapR M5 Edition.
	SupportedProducts []*string `type:"list"`
...
type RunJobFlowOutput struct {
	...
	// An unique identifier for the server flow.
	JobFlowId *string `type:"string"`
}
...
type SetVisibleToAllUsersInput struct {
	...
	// Identifiers of the server flows to receive the new visibility setting.
	//
	// JobFlowIds is a required field
	JobFlowIds []*string `type:"list" required:"true"`
...
type Step struct {
	...
	// The Hadoop server configuration of the cluster step.
...
// Specification of a cluster (server flow) step.
...
type TerminateJobFlowsInput struct {
	...
	// A list of server flows to be shutdown.
	//
	// JobFlowIds is a required field
	JobFlowIds []*string `type:"list" required:"true"`
}
