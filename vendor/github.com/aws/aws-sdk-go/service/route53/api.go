...
//    and then create a health check that is based on the store of the alarm.
...
type ChangeInfo struct {
	...
	// The current store of the request. PENDING indicates that this request has
...
type GeoLocation struct {
	...
	// The code for the subdivision, for example, a store in the United States or
...
type GeoLocationDetails struct {
	...
	// The full name of the subdivision, for example, a store in the United States
...
type HealthCheckConfig struct {
	...
	// store, the status that you want Amazon Route 53 to assign to the health check:
	//
	...
	//    store. For new health checks that have no last known status, the default
	...
	//    alarm. If the store of the alarm is OK, the health check is considered
	//    healthy. If the store is ALARM, the health check is considered unhealthy.
	//    If CloudWatch doesn't have sufficient data to determine whether the store
...
type ListGeoLocationsInput struct {
	...
	// The code for the subdivision (for example, store or province) with which
...
type ResourceRecordSet struct {
	...
	//    health checks)
	//
	//    * By determining the current store of a CloudWatch alarm (CloudWatch metric
	//    health checks)
	//
	...
	// region. For example, suppose you have resource record sets for a store in
	// the United States, for the United States, for North America, and for all
	// locations. If the endpoint for the store resource record set is unhealthy,
