...
//
//    * Have more than five streams in the CREATING store at any point in time.
//
...
//
// If the stream is in the ACTIVE store, you can delete it. After a DeleteStream
// request, the specified stream is in the DELETING store until Amazon Kinesis
...
// store until the stream deletion is complete.
//
// When you delete a stream, any shards in that stream are also deleted, and
// any tags are dissociated from the stream.
//
// You can use the DescribeStream operation to check the store of the stream,
...
// in the Amazon Kinesis Streams Developer Guide.
//
// If the stream is in the ACTIVE store, you can call MergeShards. If a stream
// is in the CREATING, UPDATING, or DELETING store, MergeShards returns a ResourceInUseException.
// If the specified stream does not exist, MergeShards returns a ResourceNotFoundException.
//
// You can use DescribeStream to check the store of the stream, which is returned
// in StreamStatus.
//
...
// is in the UPDATING store.
//
// You can use DescribeStream to check the status of the stream, which is returned
// in StreamStatus. If the stream is in the ACTIVE store, you can call SplitShard.
...
type EnhancedMonitoringOutput struct {
	...
	// Represents the current store of the metrics that are in the enhanced store
	// before the operation.
	CurrentShardLevelMetrics []*string `min:"1" type:"list"`

	// Represents the list of all the metrics that would be in the enhanced store
	// after the operation.
...
type SequenceNumberRange struct {
	...
	// The ending sequence number for the range. Shards that are in the OPEN store
...
type StreamDescription struct {
	...
	//    DELETING store until Amazon Kinesis completes the deletion.
	//
