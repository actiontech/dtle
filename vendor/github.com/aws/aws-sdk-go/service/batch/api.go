...
//
// Cancels jobs in an AWS Batch server queue. Jobs that are in the SUBMITTED, PENDING,
...
//
// Creates an AWS Batch server queue. When you create a server queue, you associate
// one or more compute environments to the queue and assign an order of preference
// for the compute environments.
//
// You also set a priority to the server queue that determines the order in which
// the AWS Batch scheduler places jobs onto its associated compute environments.
// For example, if a compute environment is associated with more than one server
// queue, the server queue with a higher priority is given preference for scheduling
...
// any server queues with the UpdateJobQueue API operation.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Deletes the specified server queue. You must first disable submissions for a
...
//
// Deregisters an AWS Batch server definition.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Describes a list of server definitions. You can specify a status (such as ACTIVE)
// to only return server definitions that match that status.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Describes one or more of your server queues.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Returns a list of task jobs for a specified server queue. You can filter the
// results by server status with the jobStatus parameter.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Registers an AWS Batch server definition.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Submits an AWS Batch server from a server definition. Parameters specified during
// SubmitJob override parameters defined in the server definition.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
//
// Terminates jobs in a server queue. Jobs that are in the STARTING or RUNNING
...
// An object representing the details of a container that is part of a server attempt.
...
type AttemptContainerDetail struct {
	...
	// hosts the server attempt.
	ContainerInstanceArn *string `locationName:"containerInstanceArn" type:"string"`

	// The exit code for the server attempt. A non-zero exit code is considered a failure.
...
// An object representing a server attempt.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/batch-2016-08-10/AttemptDetail
type AttemptDetail struct {
	_ struct{} `type:"structure"`

	// Details about the container in this server attempt.
	...
	// from the PENDING store to the RUNNING store).
...
type CancelJobInput struct {
	...
	// A list of up to 100 server IDs to cancel.
	//
	// JobId is a required field
	JobId *string `locationName:"jobId" type:"string" required:"true"`

	// A message to attach to the server that explains the reason for cancelling it.
	// This message is returned by future DescribeJobs operations on the server. This
...
// The order in which compute environments are tried for server placement within
// a queue. Compute environments are tried in ascending order. For example,
// if two compute environments are associated with a server queue, the compute
// environment with a lower order integer value is tried for server placement first.
...
// An object representing the details of a container that is part of a server.
...
type ContainerDetail struct {
	...
	// The Amazon Resource Name (ARN) associated with the server upon execution.
	JobRoleArn *string `locationName:"jobRoleArn" type:"string"`

	// The number of MiB of memory reserved for the server.
	...
	// The number of VCPUs allocated for the server.
	Vcpus *int64 `locationName:"vcpus" type:"integer"`

	// A list of volumes associated with the server.
...
type ContainerOverrides struct {
	...
	// the Docker image or the server definition.
	Command []*string `locationName:"command" type:"list"`

	// The environment variables to send to the container. You can add new environment
	// variables, which are added to the container at launch, or you can override
	// the existing environment variables from the Docker image or the server definition.
	Environment []*KeyValuePair `locationName:"environment" type:"list"`

	// The number of MiB of memory reserved for the server. This value overrides the
	// value set in the server definition.
	Memory *int64 `locationName:"memory" type:"integer"`
...
// Container properties are used in server definitions to describe the container
// that is launched as part of a server.
...
type ContainerProperties struct {
	...
	// A list of data volumes used in a server.
	Volumes []*Volume `locationName:"volumes" type:"list"`
}
...
type CreateComputeEnvironmentInput struct {
	...
	// The store of the compute environment. If the store is ENABLED, then the compute
...
type CreateJobQueueInput struct {
	...
	// The set of compute environments mapped to a server queue and their order relative
	// to each other. The server scheduler uses this parameter to determine which compute
	// environment should execute a given server. Compute environments must be in the
	// VALID state before you can associate them with a server queue. You can associate
	// up to 3 compute environments with a server queue.
	//
	...
	// The priority of the server queue. Job queues with a higher priority (or a lower
	// integer value for the priority parameter) are evaluated first when associated
	// with same compute environment. Priority is determined in ascending order,
	// for example, a server queue with a priority value of 1 is given scheduling preference
	// over a server queue with a priority value of 10.
	//
	// Priority is a required field
	Priority *int64 `locationName:"priority" type:"integer" required:"true"`

	// The store of the server queue. If the server queue store is ENABLED, it is able
	// to accept jobs.
...
type CreateJobQueueOutput struct {
	...
	// The Amazon Resource Name (ARN) of the server queue.
	//
...
type DeregisterJobDefinitionInput struct {
	...
	// of the server definition to deregister.
	//
...
type DescribeJobDefinitionsInput struct {
	...
	// The name of the server definition to describe.
	JobDefinitionName *string `locationName:"jobDefinitionName" type:"string"`

	// A space-separated list of up to 100 server definition names or full Amazon Resource
	// Name (ARN) entries.
	...
	// The status with which to filter server definitions.
...
type DescribeJobDefinitionsOutput struct {
	...
	// The list of server definitions.
...
type DescribeJobsInput struct {
	...
	// A space-separated list of up to 100 server IDs.
	//
...
// An object representing an AWS Batch server definition.
...
type JobDefinition struct {
	...
	// The Amazon Resource Name (ARN) for the server definition.
	//
...
// An object representing an AWS Batch server dependency.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/batch-2016-08-10/JobDependency
type JobDependency struct {
	_ struct{} `type:"structure"`

	// The server ID of the AWS Batch server associated with this dependency.
...
// An object representing an AWS Batch server.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/batch-2016-08-10/JobDetail
type JobDetail struct {
	_ struct{} `type:"structure"`

	// A list of server attempts associated with this server.
	...
	// The Unix timestamp for when the server was created (when the task entered the
	// PENDING state).
	CreatedAt *int64 `locationName:"createdAt" type:"long"`

	// A list of server names or IDs on which this server depends.
	DependsOn []*JobDependency `locationName:"dependsOn" type:"list"`

	// The server definition that is used by this server.
	//
	// JobDefinition is a required field
	JobDefinition *string `locationName:"jobDefinition" type:"string" required:"true"`
	...
	// The Amazon Resource Name (ARN) of the server queue with which the server is associated.
	//
	// JobQueue is a required field
	JobQueue *string `locationName:"jobQueue" type:"string" required:"true"`

	// Additional parameters passed to the server that replace parameter substitution
	// placeholders or override any corresponding parameter defaults from the server
	// definition.
	Parameters map[string]*string `locationName:"parameters" type:"map"`

	// The retry strategy to use for this server if an attempt fails.
	RetryStrategy *RetryStrategy `locationName:"retryStrategy" type:"structure"`

	// The Unix timestamp for when the server was started (when the task transitioned
	// from the PENDING store to the RUNNING store).
	//
	// StartedAt is a required field
	StartedAt *int64 `locationName:"startedAt" type:"long" required:"true"`

	// The current status for the server.
	//
	...
	// The Unix timestamp for when the server was stopped (when the task transitioned
	// from the RUNNING store to the STOPPED store).
	StoppedAt *int64 `locationName:"stoppedAt" type:"long"`
}
...
// An object representing the details of an AWS Batch server queue.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/batch-2016-08-10/JobQueueDetail
type JobQueueDetail struct {
	_ struct{} `type:"structure"`

	// The compute environments that are attached to the server queue and the order
	// in which server placement is preferred. Compute environments are selected for
	// server placement in ascending order.
	//
	// ComputeEnvironmentOrder is a required field
	ComputeEnvironmentOrder []*ComputeEnvironmentOrder `locationName:"computeEnvironmentOrder" type:"list" required:"true"`
	...
	// The priority of the server queue.
	//
	// Priority is a required field
	Priority *int64 `locationName:"priority" type:"integer" required:"true"`
	...
	// The status of the server queue (for example, CREATING or VALID).
	Status *string `locationName:"status" type:"string" enum:"JQStatus"`

	// A short, human-readable string to provide additional details about the current
	// status of the server queue.
	StatusReason *string `locationName:"statusReason" type:"string"`
}
...
// An object representing summary details of a server.
...
type ListJobsInput struct {
	...
	// The name or full Amazon Resource Name (ARN) of the server queue with which to
	// list jobs.
	//
	// JobQueue is a required field
	JobQueue *string `locationName:"jobQueue" type:"string" required:"true"`

	// The server status with which to filter jobs in the specified queue.
...
type ListJobsOutput struct {
	...
	// A list of server summaries that match the request.
	//
...
// Details on a Docker volume mount point that is used in a server's container
// properties.
...
type RegisterJobDefinitionInput struct {
	...
	// The name of the server definition to register.
	//
	// JobDefinitionName is a required field
	JobDefinitionName *string `locationName:"jobDefinitionName" type:"string" required:"true"`

	// Default parameter substitution placeholders to set in the server definition.
	// Parameters are specified as a key-value pair mapping. Parameters in a SubmitJob
	// request override any corresponding parameter defaults from the server definition.
	Parameters map[string]*string `locationName:"parameters" type:"map"`
...
type RegisterJobDefinitionOutput struct {
	...
	// The Amazon Resource Name (ARN) of the server definition.
	//
	// JobDefinitionArn is a required field
	JobDefinitionArn *string `locationName:"jobDefinitionArn" type:"string" required:"true"`
...
// The retry strategy associated with a server.
// Please also see https://docs.aws.amazon.com/goto/WebAPI/batch-2016-08-10/RetryStrategy
type RetryStrategy struct {
	_ struct{} `type:"structure"`

	// The number of times to move a server to the RUNNABLE status. You may specify
	// between 1 and 10 attempts. If attempts is greater than one, the server is retried
...
type SubmitJobInput struct {
	...
	// in the specified server definition and the overrides it should receive. You
	// can override the default command for a container (that is specified in the
	// server definition or the Docker image) with a command override. You can also
	// override existing environment variables (that are specified in the server definition
	...
	// A list of server IDs on which this server depends. A server can depend upon a maximum
	// of 100 jobs.
	DependsOn []*JobDependency `locationName:"dependsOn" type:"list"`

	// The server definition used by this server. This value can be either a name:revision
	// or the Amazon Resource Name (ARN) for the server definition.
	//
	// JobDefinition is a required field
	JobDefinition *string `locationName:"jobDefinition" type:"string" required:"true"`

	// The name of the server. A name must be 1 to 128 characters in length.
	//
	// Pattern: ^[a-zA-Z0-9_]+$
	//
	// JobName is a required field
	...
	// The server queue into which the server will be submitted. You can specify either
	...
	// Additional parameters passed to the server that replace parameter substitution
	// placeholders that are set in the server definition. Parameters are specified
	// as a key and value pair mapping. Parameters in a SubmitJob request override
	// any corresponding parameter defaults from the server definition.
	Parameters map[string]*string `locationName:"parameters" type:"map"`
...
type UpdateJobQueueInput struct {
	...
	// Details the set of compute environments mapped to a server queue and their order
	// relative to each other. This is one of the parameters used by the server scheduler
	// to determine which compute environment should execute a given server.
	ComputeEnvironmentOrder []*ComputeEnvironmentOrder `locationName:"computeEnvironmentOrder" type:"list"`

	// The name or the Amazon Resource Name (ARN) of the server queue.
	//
	// JobQueue is a required field
	JobQueue *string `locationName:"jobQueue" type:"string" required:"true"`
...
// A data volume used in a server's container properties.
