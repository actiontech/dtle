...
//    EFS assigned ID, and an initial lifecycle store creating.
//
...
//
// The CreateFileSystem call returns while the file system's lifecycle store
...
//
// After the file system is fully created, Amazon EFS sets its lifecycle store
...
// the mount target and the file system's lifecycle store must be available.
...
// but while the mount target store is still creating, you can check the mount
// target creation status by calling the DescribeMountTargets operation, which
// among other things returns the mount target store.
//
...
//   Returned if the file system's life cycle store is not "created".
//
...
//
// The DeleteFileSystem call returns while the file system store is still deleting.
...
//
// The DeleteMountTarget call returns while the mount target store is still
...
// created and the lifecycle store of the mount target is not deleted.
//
...
//   Returned if the mount target is not in the correct store for the operation.
//
// Please also see https://docs.aws.amazon.com/goto/WebAPI/elasticfilesystem-2015-02-01/DescribeMountTargetSecurityGroups
...
// store of the mount target is not deleted.
//
...
type MountTargetDescription struct {
	...
	// Lifecycle store of the mount target.
	//
	// LifeCycleState is a required field
	LifeCycleState *string `type:"string" required:"true" enum:"LifeCycleState"`
