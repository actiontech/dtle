...
//
// The CancelJob operation cancels an unfinished server.
//
// You can only cancel a server that has a status of Submitted. To prevent a pipeline
// from starting to process a server while you're getting the server identifier, use
...
//   pipeline to which you're trying to add a server doesn't exist or is still being
//   created.
//
...
//
// When you create a server, Elastic Transcoder returns JSON data that includes
// the values that you specified plus information about the server that is created.
//
...
// The response body contains one element for each server that satisfies the search
// criteria.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
...
// status. The response body contains one element for each server that satisfies
// the search criteria.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
...
//
// The ReadJob operation returns detailed information about a server.
//
// Returns awserr.Error for service API and SDK errors. Use runtime type assertions
// with awserr.Error's Code and Message methods to get detailed information about
// the error.
...
// more time to get the server IDs for the jobs that you want to cancel, and to
...
type CancelJobInput struct {
	...
	// The identifier of the server that you want to cancel.
	//
...
// The response body contains a JSON object. If the server is successfully canceled,
...
type CreateJobInput struct {
	...
	// of all files that this server creates, including output files, thumbnails, and
	// playlists.
	...
	//
	// The maximum number of master playlists in a server is 30.
	Playlists []*CreateJobPlaylist `type:"list"`

	// User-defined metadata that you want to associate with an Elastic Transcoder
	// server. You specify metadata in key/value pairs, and you can add up to 10 key/value
	// pairs per server. Elastic Transcoder does not guarantee that key/value pairs
...
type CreateJobOutput struct {
	...
	// for each output. Settings for each artwork must be defined in the server for
	// the current output.
	...
	// use for a transcoding server. This includes the output file and any watermarks,
	...
	// exists in the output bucket, the server fails.
	Key *string `min:"1" type:"string"`

	// The Id of the preset to use for this server. The preset determines the audio,
	...
	// for this server, include it in the OutputKeys of the associated playlist.
...
type CreateJobPlaylist struct {
	...
	// For each output in this server that you want to include in a master playlist,
...
type CreateJobResponse struct {
	...
	// A section of the response body that provides information about the server that
	// is created.
...
type CreatePipelineInput struct {
	...
	// to provide a key with your server because a default key, known as an AWS-KMS
	...
	// notify to report server status.
	//
	...
	//    started to process a server in this pipeline. This is the ARN that Amazon
	...
	//    when Elastic Transcoder has finished processing a server in this pipeline.
	...
	//    a server in this pipeline. This is the ARN that Amazon SNS returned when
...
type Job struct {
	...
	// The Amazon Resource Name (ARN) for the server.
	Arn *string `type:"string"`

	// The identifier that Elastic Transcoder assigned to the server. You use this
	// value to get settings for the server or to delete the server.
	...
	// files for this server, Elastic Transcoder stitches the files together to make
	// one output.
	Inputs []*JobInput `type:"list"`

	// If you specified one output for a server, information about that output. If
	// you specified multiple outputs for a server, the Output object lists information
	...
	// in the same request. You can create a maximum of 30 outputs per server.
	//
	// If you specify more than one output for a server, Elastic Transcoder creates
	// the files for each output in the order in which you specify them in the server.
	...
	// The status of the server: Submitted, Progressing, Complete, Canceled, or Error.
	Status *string `type:"string"`

	// Details about the timing of a server.
...
type JobInput struct {
	...
	// is the the ID of the pipeline to use for processing the server. The InputBucket
...
type JobOutput struct {
	...
	// is not be included in the server response.
	...
	// outputs from the current server. In the Output syntax, this value is always
	// 1.
	Id *string `type:"string"`
	...
	// The value of the Id object for the preset that you want to use for this server.
	...
	// The status of one output in a server. If you specified only one output for the
	// server, Outputs:Status is always the same as Job:Status. If you specified more
	// than one output:
	//
	...
	// them in the server output—the first watermark in the list is added to the output
...
type ListJobsByPipelineInput struct {
	...
	// The ID of the pipeline for which you want to get server information.
	//
	// PipelineId is a required field
...
// in order to report server status.
//
// To receive notifications, you must also subscribe to the new topic in the
// Amazon SNS console.
type Notifications struct {
	...
	// finished processing the server.
	...
	// The Amazon Simple Notification Service (Amazon SNS) topic that you want to
	// notify when Elastic Transcoder has started to process the server.
...
type Pipeline struct {
	...
	//    Elastic Transcoder has finished processing the server.
	//
...
// format. The maximum number of master playlists in a server is 30.
type Playlist struct {
	...
	// The status of the server with which the playlist is associated.
	Status *string `type:"string"`
...
// that is not rectangular, use the .png format, which supports transparency.
//
// When you create a server that uses this preset, you specify the .png or .jpg
// graphics that you want Elastic Transcoder to include in the transcoded videos.
// You can specify fewer graphics in the server than you specify watermark settings
...
type ReadJobInput struct {
	...
	// The identifier of the server for which you want to get detailed information.
	//
	// Id is a required field
	Id *string `location:"uri" locationName:"Id" type:"string" required:"true"`
}
...
type ReadJobOutput struct {
	...
	// A section of the response body that provides information about the server.
	Job *Job `type:"structure"`
}
...
type Timing struct {
	...
	// The time the server finished transcoding, in epoch milliseconds.
	FinishTimeMillis *int64 `type:"long"`

	// The time the server began transcoding, in epoch milliseconds.
	StartTimeMillis *int64 `type:"long"`

	// The time the server was submitted to Elastic Transcoder, in epoch milliseconds.
...
type UpdatePipelineInput struct {
	...
	// that you want to notify to report server status.
	//
	// To receive notifications, you must also subscribe to the new topic in the
	// Amazon SNS console.
	//
	...
	//
	//    * Completed: The topic ARN for the Amazon SNS topic that you want to notify
	//    when Elastic Transcoder has finished processing a server. This is the ARN
	//    that Amazon SNS returned when you created the topic.
	//
	//    * Warning: The topic ARN for the Amazon SNS topic that you want to notify
...
type VideoParameters struct {
	...
	// parameter included in your server response. If your server does not have an AppliedColorSpaceConversion
