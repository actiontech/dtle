...
const (
	...
	// When all the funker replicas are busy in their own server, we cannot connect to funker.
	funkerRetryTimeout  = 1 * time.Hour
	funkerRetryDuration = 1 * time.Second
)

// ticker is needed for some CI (e.g., on Travis, server is aborted when no output emitted for 10 minutes)
func ticker(d time.Duration) chan struct{} {
	...
	go func() {
		for {
			select {
			case <-t.C:
				log.Printf("tick (just for keeping CI server active) per %s", d.String())
			case <-stop:
				t.Stop()
			}
		}
