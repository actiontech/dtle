...
```
For `server="foo"` (common label to differentiate between Prometheus monitoring targets), calculate the
...
```
For `server="foo"`, calculate the per-`grpc_service` rate of `unary` (1:1) RPCs that failed, i.e. the
 ...
 * 100.0
```
For `server="foo"`, calculate the percentage of failed requests by service. It's easy to notice that
...
```
For `server="foo"` what is the `grpc_service`-wide `10m` average of messages returned for all `
...
```
For `server="foo"`, returns an 99%-tile [quantile estimation](https://prometheus.io/docs/practices/histograms/#quantiles)
...
```
For `server="foo"` calculate the by-`grpc_service` fraction of slow requests that took longer than `0.25`
