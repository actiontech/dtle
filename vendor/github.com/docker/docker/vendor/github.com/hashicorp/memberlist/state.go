...
// NodeState is used to manage our store view of another node
type nodeState struct {
	Node
	Incarnation uint32        // Last known incarnation number
	State       nodeStateType // Current store
	StateChange time.Time     // Time last store change happened
}
...
// pushPull is invoked periodically to randomly perform a complete store
// exchange. Used to ensure a high level of convergence, but is also
// reasonably expensive as the entire store of this node is exchanged
// with the other node.
...
// pushPullNode does a complete store exchange with a specific node.
...
func (m *Memberlist) aliveNode(a *alive, notify chan struct{}, bootstrap bool) {
	...
	// Store the old store and meta data
	...
	if !bootstrap && isLocalNode {
		...
		// Update the store and incarnation number
