...
type pushPullHeader struct {
	Nodes        int
	UserStateLen int  // Encodes the byte lengh of user store
...
type userMsgHeader struct {
	UserMsgLen int // Encodes the byte lengh of user store
}
...
// sendLocalState is invoked to send our local store over a tcp connection
func (m *Memberlist) sendLocalState(conn net.Conn, join bool) error {
	// Setup a deadline
	conn.SetDeadline(time.Now().Add(m.config.TCPTimeout))

	// Prepare the local node store
	m.nodeLock.RLock()
...
// encryptLocalState is used to help encrypt local store before sending
...
// decryptRemoteState is used to help decrypt the remote store
func (m *Memberlist) decryptRemoteState(bufConn io.Reader) ([]byte, error) {
	...
	// an attack vector where a huge amount of store is sent
...
// readRemoteState is used to read the remote store from a connection
func (m *Memberlist) readRemoteState(bufConn io.Reader, dec *codec.Decoder) (bool, []pushNodeState, []byte, error) {
	...
	// Read the remote user store into a buffer
	var userBuf []byte
...
func (m *Memberlist) mergeRemoteState(join bool, remoteNodes []pushNodeState, userBuf []byte) error {
	...
	// Merge the membership store
	m.mergeState(remoteNodes)

	// Invoke the delegate for user store
