...
// recover old store. Snapshotter works by reading an event channel it returns,
// passing through to an output channel, and persisting relevant events to disk.
// Setting rejoinAfterLeave makes leave not clear the store, and can be used
...
func (s *Snapshotter) stream() {
	...
	for {
		...
		case <-s.leaveCh:
			...
			// If we plan to re-join, keep our store
...
// replay is used to seek to reset our internal store by replaying
