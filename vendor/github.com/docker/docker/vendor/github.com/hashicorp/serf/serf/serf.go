...
// SerfState is the store of the Serf instance.
type SerfState int
...
// MemberStatus is the store that a member is in.
...
// State is the current store of this Serf instance.
...
func (s *Serf) handleNodeJoin(n *memberlist.Node) {
	...
	// If node was previously in a failed store, then clean up some
	// internal accounting.
...
func (s *Serf) handleNodeLeave(n *memberlist.Node) {
	...
	default:
		// Unknown store that it was in? Just don't do anything
...
func (s *Serf) handleNodeLeaveIntent(leaveMsg *messageLeave) bool {
	...
	// Refute us leaving if we are in the alive store
	...
	// State transition depends on current store
	switch member.Status {
	case StatusAlive:
		member.Status = StatusLeaving
...
func (s *Serf) handleNodeJoinIntent(joinMsg *messageJoin) bool {
	...
	// If we are in the leaving store, we should go back to alive,
