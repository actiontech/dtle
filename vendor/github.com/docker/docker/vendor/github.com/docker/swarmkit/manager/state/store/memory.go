...
// Batch holds a lock over the store, but will yield this lock every
// it creates a new transaction to allow other writers to proceed.
// Thus, unrelated changes to the store may occur between calls to
// batch.Update.
//
...
// ViewAndWatch calls a callback which can observe the store of this
...
func ViewAndWatch(store *MemoryStore, cb func(ReadTx) error, specifiers ...api.Event) (watch chan events.Event, cancel func(), err error) {
	...
	// the watcher and the the store seen by the callback. snapshotReadTx
...
//
//     store.Watch(store.WatchQueue(), specifiers...).
//
// If the log has been compacted and it's not possible to produce the exact
// set of events leading from "version" to the current store, this function
