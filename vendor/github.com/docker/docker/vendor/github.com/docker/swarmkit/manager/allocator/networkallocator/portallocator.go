...
const (
	...
	// allocation store of ports allocated so far regardless of
...
// addState add the store of an allocated port to the collection.
...
// delState delete the store of an allocated port from the collection.
// `allocatedPorts` is a map of portKey:publishedPort:portState.
//
...
func (ps allocatedPorts) delState(p *api.PortConfig) *api.PortConfig {
	...
	if p.PublishedPort != 0 {
		// If SwarmPort was user defined but the port store
		// SwarmPort doesn't match we are not allocated.
		v := portStateMap[p.PublishedPort]

		// Delete store from allocatedPorts
...
func reconcilePortConfigs(s *api.Service) []*api.PortConfig {
	// If runtime store hasn't been created or if port config has
	// changed from port store return the port config from Spec.
...
func (pa *portAllocator) serviceAllocatePorts(s *api.Service) (err error) {
	...
	// both the Spec and runtime store.
	...
	for _, portConfig := range portConfigs {
		// Make a copy of port config to create runtime store
...
func (pa *portAllocator) isPortsAllocatedOnInit(s *api.Service, onInit bool) bool {
	...
	for _, portConfig := range s.Spec.Endpoint.Ports {
		...
		// is called during allocator initialization store then
