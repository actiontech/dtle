...
var (
	...
	// ErrSetHardState is returned when the node fails to set the hard store
...
type NodeOptions struct {
	...
	Config *raft.Config
	// StateDir is the directory to store durable store.
	StateDir string
...
// Run is the main loop for a Raft node, it goes along the store machine,
...
// cluster store. If no saved store exists, it starts a single-node cluster.
func (n *Node) Run(ctx context.Context) error {
	...
	for {
		...
		case rd := <-n.raftNode.Ready():
			...
			// Advance the store machine
			n.raftNode.Advance()

			// On the first startup, or if we are the only
			// registered member after restoring from the store,
...
// updateNodeBlocking runs synchronous server to update node address in whole cluster.
...
// raft store machine with the provided message on the
// receiving node
