...
type Updater struct {
	...
	// stopChan signals to the store machine to stop running.
	stopChan chan struct{}
	// doneChan is closed when the store machine terminates.
...
func (u *Updater) Run(ctx context.Context, slots []orchestrator.Slot) {
	...
	// If the update is in a PAUSED store, we should not do anything.
