	...
	for _, t := range tasks {
		...
		if taskReadyForNetworkVote(t, s, nc) {
			...
			if a.taskAllocateVote(networkVoter, t.ID) {
				...
				// allocators server is done. Immediately cast a vote so
				// that the task can be moved to the PENDING store as
				// soon as possible.
...
// vote to move it to PENDING store.
...
func (a *Allocator) doTaskAlloc(ctx context.Context, ev events.Event) {
	...
	// If we are already in allocated store, there is
	...
	if t.ServiceID != "" {
		...
		if s == nil {
			...
			// is either dead or the desired store is set
...
// the published ports in host mode: It resets the runtime store ports (s.Endpoint.Ports)
// to the current ingress mode runtime store ports plus the newly configured publish mode ports,
...
func (a *Allocator) allocateTask(ctx context.Context, t *api.Task) (err error) {
	...
	// PENDING store on top of the latest store store.
	if a.taskAllocateVote(networkVoter, t.ID) {
		if t.Status.State < api.TaskStatePending {
			updateTaskStatus(t, api.TaskStatePending, allocatedStatusMessage)
			taskUpdated = true
