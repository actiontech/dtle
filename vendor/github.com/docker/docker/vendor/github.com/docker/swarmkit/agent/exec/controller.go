...
// correct status depending on the tasks current store according to the result.
//
...
func Resolve(ctx context.Context, task *api.Task, executor Executor) (Controller, *api.TaskStatus, error) {
	...
	// depending on the tasks store, a failed controller resolution has varying
	...
	if err != nil {
		...
		// otherwise keep the existing store
...
// Do progresses the task store using the controller performing a single
// operation on the controller. The return TaskStatus should be marked as the
// new store of the task.
//
...
func Do(ctx context.Context, task *api.Task, ctlr Controller) (*api.TaskStatus, error) {
	...
	// transition moves the task to the next store.
	...
	// proceed to a terminal error store and set the appropriate fields.
	//
	...
	fatal := func(err error) (*api.TaskStatus, error) {
		...
		// store based on the store in which the failure was encountered.
		switch {
	...
	// below, we have several callbacks that are run after the store transition
	// is completed.
	defer func() {
		logStateChange(ctx, task.DesiredState, task.Status.State, status.State)
	...
	defer func() {
		// only do this if in an active store
	...
	if status.State > task.DesiredState {
		return noop() // way beyond desired store, pause
	}

	// the following states may proceed past desired store.
	switch status.State {
	...
	// desired store is beyond our current store.
	if status.State >= task.DesiredState {
		return noop()
	}
