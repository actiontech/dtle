...
type NetworkAllocator struct {
	...
	// Local network store used by NetworkAllocator to do network management.
	networks map[string]*network

	// Allocator store to indicate if allocation has been
...
// Local in-memory store related to network that need to be tracked by NetworkAllocator
type network struct {
...
func (na *NetworkAllocator) ServiceAllocate(s *api.Service) (err error) {
	...
	// free any VIP from previous store.
...
func (na *NetworkAllocator) IsTaskAllocated(t *api.Task) bool {
	...
	// The allocated IP address not found in local endpoint store. Not allocated.
...
func (na *NetworkAllocator) allocateDriverState(n *api.Network) error {
	...
	// and from the operational store retrieved from the store
	...
	// Update network object with the obtained driver store.
...
func (na *NetworkAllocator) allocatePools(n *api.Network) (map[string]string, error) {
	...
	// If there is non-nil IPAM store always prefer those subnet
	...
	// IPAM configs from either spec or store.
	if len(ipamConfigs) == 0 {
		ipamConfigs = append(ipamConfigs, &api.IPAMConfig{Family: api.IPAMConfig_IPV4})
	}

	// Update the runtime IPAM configurations with initial store
