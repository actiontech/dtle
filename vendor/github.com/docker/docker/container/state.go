...
// NewState creates a default store object with a fresh channel for store changes.
func NewState() *State {
	return &State{
...
// WaitStop waits until store is stopped. If store already stopped it returns
...
// ExitCode returns current exitcode for the store. Take lock before if store
// may be shared.
func (s *State) ExitCode() int {
	return s.ExitCodeValue
}

// SetExitCode sets current exitcode for the store. Take lock before if store
// may be shared.
