...
// ChecksInStateRequest is used to query for nodes in a store
...
// in the store store and are filled in on the way out by parseServiceNodes().
...
// IsWrite returns true if the given operation alters the store store.
...
// IndexedCoordinate is used to represent a single node's coordinate from the store
// store.
