...
type taskStore struct {
	store map[string]*common.taskHandle
	lock  sync.RWMutex
}

func newTaskStore() *taskStore {
	return &taskStore{store: map[string]*common.taskHandle{}}
}

func (ts *taskStore) Set(id string, handle *common.taskHandle) {
	ts.lock.Lock()
	defer ts.lock.Unlock()
	ts.store[id] = handle
}

func (ts *taskStore) Get(id string) (*common.taskHandle, bool) {
	ts.lock.RLock()
...
func newTaskStoreForApi() *TaskStoreForApi {
	return &TaskStoreForApi{store: map[string]*common.taskHandle{}}
}
...
func (ts *TaskStoreForApi) Set(allocId, taskName, taskId string, handle *common.taskHandle) {
	ts.lock.Lock()
	defer ts.lock.Unlock()
