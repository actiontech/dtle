...
func NewDriver(logger g.LoggerType) drivers.DriverPlugin {
	...
	AllocIdTaskNameToTaskHandler = common.newTaskStoreForApi()
...
func (d *Driver) StartTask(cfg *drivers.TaskConfig) (*drivers.TaskHandle, *drivers.DriverNetwork, error) {
	...
	h := common.newDtleTaskHandle(d.logger, cfg, drivers.TaskStateRunning, time.Now().Round(time.Millisecond))
...
func (d *Driver) handleWait(ctx context.Context, handle *common.taskHandle, ch chan *drivers.ExitResult) {
	defer close(ch)
