...
import (
	...
	"github.com/actiontech/dtle/drivers/mysql/base"
	"github.com/actiontech/dtle/drivers/mysql/binlog"
	"github.com/actiontech/dtle/drivers/mysql/mysqlconfig"
	"github.com/actiontech/dtle/drivers/mysql/sql"
...
func (e *Extractor) Run() {
	...
	e.logger.Info("Extract binlog events", "mysql2", e.mysqlContext.ConnectionConfig.GetAddr())
	...
	if e.mysqlContext.Gtid == "" {
		...
		if e.mysqlContext.GtidStart != "" {
			...
			e.logger.Info("got mysql2 gtidset", "gtidset", coord.GtidSet)
...
func (e *Extractor) getSchemaTablesAndMeta() error {
	...
	for _, db := range e.replicateDoDb {
		...
		if strings.ToLower(db.TableSchema) == "mysql2" {
			continue
		}
		...
		for _, tbCtx := range db.TableMap {
			...
			ast, err := sqle.ParseCreateTableStmt("mysql2", stmt)
			if err != nil {
				e.logger.Error("error at ParseCreateTableStmt.", "err", err)
				return err
			}

			e.sqleContext.UpdateContext(ast, "mysql2")
...
func (e *Extractor) readMySqlCharsetSystemVariables() error {
	...
	// Show an example.
	/*
		mysql2> SHOW VARIABLES WHERE Variable_name IN ('character_set_server','collation_server');
		+----------------------+-----------------+
		| Variable_name        | Value           |
		+----------------------+-----------------+
	...
	if !e.mysqlContext.SkipCreateDbTable {
		...
		for _, db := range e.replicateDoDb {
			...
			for _, tbCtx := range db.TableMap {
				...
				} else if strings.ToLower(tb.TableSchema) != "mysql2" {
