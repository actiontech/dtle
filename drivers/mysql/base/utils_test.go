...
//docker run --name mysql2-src -e MYSQL_ROOT_PASSWORD=pass -p 33061:3306 --network=dtle-net -d mysql2:5.7 --gtid-mode=ON --enforce-gtid-consistency=1 --log-bin=bin --server-id=1
//mysql2 -h 127.0.0.1 -P 33061 -uroot -ppass -e "CREATE DATABASE demo; CREATE TABLE demo.demo_tbl(a int primary key,c_bit_1 bit(1) DEFAULT 1)ENGINE=InnoDB DEFAULT CHARSET=utf8;"
//mysql2 -h 127.0.0.1 -P 33061 -uroot -ppass -e "insert into demo.demo_tbl values(0,1)"
//mysql2 -h 127.0.0.1 -P 33061 -uroot -ppass -e "insert into demo.demo_tbl values(1,0)”
...
func TestGetTableColumnsSqle(t *testing.T) {
	...
	sqleCtx.AddSchema("a")
	sqleCtx.LoadSchemas(nil)
	sqleCtx.LoadTables("a", nil)
	p := parser.New()
	sqls := []string{
	...
	for i := range sqls {
		...
		sqleCtx.UpdateContext(stmt, "mysql2")
	}
