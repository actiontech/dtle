...
import (
	...
	ubase "github.com/actiontech/dtle/drivers/mysql/base"
...
func (i *Inspector) ValidateOriginalTable(databaseName, tableName string, table *common.Table) (err error) {
	...
	table.ColumnMap = umconf.BuildColumnMapIndex(table.ColumnMapFrom, table.OriginalTableColumns.Ordinals)
...
func (i *Inspector) ValidateGrants() error {
	...
	if i.mysqlContext.ExpandSyntaxSupport {
		if _, err := i.db.Query(`use mysql2`); err != nil {
...
func (i *Inspector) ValidateBinlogs() error {
	...
	i.logger.Info("Binary logs validated", "mysql2",
