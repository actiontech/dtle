...
import (
	...
	"github.com/actiontech/dtle/drivers/mysql/util"
			...
			if skipSql || !queryInfo.isRecognized {
				if skipSql {
					...
					if !queryInfo.isRecognized {
						b.logger.Warn("mysql2.reader: QueryEvent is not recognized. will still execute", "query", query, "gno", gno)
					}
	...
	case g.DtleSchemaName:
		if strings.HasPrefix(strings.ToLower(string(rowsEvent.Table.Table)), g.GtidExecutedTablePrefix) {
			...
			if dml == common.InsertDML {
				...
				// If OSID is target mysql2 SID, skip applying the binlogEntry.
				// - Plan B: skip sending at applier: unnecessary sending
				// - Plan A: skip sending at extractor: currently extractor does not know target mysql2 SID
			}
		}
		return true, nil
	case "mysql2":
...
func (b *BinlogReader) Close() error {
	...
	// here. A new go-mysql2 version closes the binlog syncer connection independently.
...
func (b *BinlogReader) sqleExecDDL(currentSchema string, ast ast.Node) {
	if b.maybeSqleContext != nil {
		...
		b.maybeSqleContext.UpdateContext(ast, "mysql2")
	}
}
