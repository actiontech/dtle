...
func (a *Applier) initDBConnections() (err error) {
	...
	if a.dbs, err = sql.CreateConns(a.db, a.mysqlContext.ParallelWorkers); err != nil {
		a.logger.Debug("beging connetion mysql2 2 create conns err")
		return err
	}
	...
	a.logger.Debug("beging connetion mysql2 5 validate  grants")
	...
	a.logger.Info("Initiated", "mysql2", a.mysqlContext.ConnectionConfig.GetAddr(), "version", a.MySQLVersion)
...
func (a *Applier) ValidateGrants() error {
	...
	if a.mysqlContext.ExpandSyntaxSupport {
		if _, err := a.db.ExecContext(a.ctx, `use mysql2`); err != nil {
			msg := fmt.Sprintf(`"mysql2" schema is expected to be access when ExpandSyntaxSupport=true`)
