...
func (c *EvalStatusCommand) Help() string {
	helpText := `
Usage: server eval-status [options] <evaluation>
...
func (c *EvalStatusCommand) Run(args []string) int {
	...
	if len(evals) > 1 {
		...
		out[0] = "ID|Triggered By|Status|Placement Failures"
	...
	if failures {
		...
		sorted := sortedTaskFromMetrics(eval.FailedTGAllocs)
		for _, t := range sorted {
			metrics := eval.FailedTGAllocs[t]

			noun := "allocation"
			if metrics.CoalescedFailures > 0 {
				noun += "s"
			}
			c.Ui.Output(fmt.Sprintf("Task %q (failed to place %d %s):", t, metrics.CoalescedFailures+1, noun))
...
func getTriggerDetails(eval *api.Evaluation) (noun, subject string) {
	...
	case "job-register", "job-deregister", "rolling-update":
