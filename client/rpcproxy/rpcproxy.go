// Package rpcproxy provides a proxy interface to Udup Servers.  The
// RPCProxy periodically shuffles which server a Udup Client communicates
// with in order to redistribute load across Udup Servers.  Udup Servers
...
// should be called only by `hashicorp/Udup`.
package rpcproxy
...
const (
	...
	// connection is established in order to rebalance load across Udup
	...
	//
	// For example, in a 10K Udup cluster with 5x servers, this default
	...
	//
	// For example, in a 100K Udup cluster with 5x servers, it will take
...
// UdupConfigInfo is an interface wrapper around this Udup Agent's
// configuration to prevents a cyclic import dependency.
type UdupConfigInfo interface {
	Datacenter() string
	RPCMajorVersion() int
	RPCMinorVersion() int
	Region() string
...
type Pinger interface {
	PingUdupServer(region string, apiMajorVersion int, s *ServerEndpoint) (bool, error)
}

// serverList is an array of Udup Servers.  The first server in the list is
// the active server.
//
...
// RPCProxy is the manager type responsible for returning and managing Udup
// addresses.
type RPCProxy struct {
	// activatedList manages the list of Udup Servers that are eligible
	...
	// the list of servers discovered via the Udup heartbeat.  Covered
	// by serverListLock.
	...
	// shutdownCh is a copy of the channel in Udup.Client
	shutdownCh chan struct{}

	logger *log.Logger

	configInfo UdupConfigInfo
...
func NewRPCProxy(logger *log.Logger, shutdownCh chan struct{}, configInfo UdupConfigInfo, connPoolPinger Pinger) *RPCProxy {
	p := &RPCProxy{
		logger:         logger,
		configInfo:     configInfo,     // can't pass *udup.Client: import cycle
		connPoolPinger: connPoolPinger, // can't pass *udup.ConnPool: import cycle
...
// SetBackupServers sets a list of Udup Servers to be used in the event that
// the Udup Agent lost contact with the list of Udup Servers provided via
// the Udup Agent's heartbeat.  If available, the backup servers are
...
// AddPrimaryServer takes the RPC address of a Udup server, creates a new
...
// the Udup Server for this Udup Agent is in the minority or the Udup
...
// NumNodes returns the estimated number of nodes according to the last Udup
// Heartbeat.
...
// across all known Udup servers (i.e. guarantee that the order of servers
// in the server list is not positively correlated with the age of a server
// in the Udup cluster).  Periodically shuffling the server list prevents
...
func (p *RPCProxy) RebalanceServers() {
	...
	for i := 0; i < len(l.L); i++ {
		...
		// and eventually removed from the list when Udup heartbeats
		// detect the failed node.
		selectedServer := l.L[0]

		ok, err := p.connPoolPinger.PingUdupServer(p.configInfo.Region(), p.configInfo.RPCMajorVersion(), selectedServer)
		if ok {
	...
	// updated list of Udup servers.  Or Consul will begin advertising a
	// new server in the Udup service (Udup server service).
...
// (i.e. was removed by Udup during a PingUdupServer() call.  Newly added
...
func (p *RPCProxy) reconcileServerList(l *serverList) bool {
	...
	// If a Udup heartbeat removed all nodes, or there is no selected
...
// rebalancing the list of Udup servers.  This maintenance only happens
...
// distribute load across all known and available Udup servers.
func (p *RPCProxy) Run() {
	for {
		select {
...
// Udup Server.  The response from Udup Client Heartbeats contain a list of
// Udup Servers that the Udup Client should use for RPC requests.
// RefreshServerLists does not rebalance its serverLists (that is handled
// elsewhere via a periodic timer).  New Udup Servers learned via the
...
// all server lists.  Udup Servers speaking a newer major or minor API
...
func (p *RPCProxy) RefreshServerLists(servers []*structs.NodeServerInfo, numNodes int32, leaderRPCAddr string) error {
	...
	for _, s := range servers {
		...
		// Udup servers in different datacenters are automatically
	...
	for k, v := range mergedPrimaryMap {
		...
		case 'o':
			...
			// TODO(sean@): Teach Udup servers how to remove
