	...
	case models.TaskStateDead:
		...
		if taskState.Failed {
			...
			if len(otherWorkers) > 0 {
				r.logger.Printf("[DEBUG] client: task %q failed, destroying other tasks in task: %v", taskName, otherTaskNames)
			}
		} else if leader {
			...
			if len(otherWorkers) > 0 {
				r.logger.Printf("[DEBUG] client: leader task %q is dead, destroying other tasks in task: %v", taskName, otherTaskNames)
			}
		}
	}
...
func (r *Allocator) Run() {
	...
	if t == nil {
		r.logger.Printf("[ERR] client: alloc '%s' for missing task '%s'", alloc.ID, alloc.Task)
