...
func (s *Server) handleConn(conn net.Conn, isTLS bool) {
	...
	if s.config.RequireTLS && !isTLS && RPCType(buf[0]) != rpcTLS {
		s.logger.Printf("[WARN] server.rpc: Non-TLS connection attempted with RequireTLS set")
		conn.Close()
		return
	}
	...
	case rpcTLS:
		if s.rpcTLS == nil {
			s.logger.Printf("[WARN] server.rpc: TLS connection attempted, server not configured for TLS")
			conn.Close()
			return
		}
