...
func (n *Node) Register(args *models.NodeRegisterRequest, reply *models.NodeUpdateResponse) error {
	...
	if err != nil {
		n.srv.logger.Printf("[ERR] server.client: Register failed: %v", err)
		return err
	}
	...
	if transitionToReady {
		...
		if err != nil {
			n.srv.logger.Printf("[ERR] server.client: eval creation failed: %v", err)
			return err
		}
	...
	if !args.Node.TerminalStatus() {
		...
		if err != nil {
			n.srv.logger.Printf("[ERR] server.client: heartbeat reset failed: %v", err)
			return err
		}
	...
	if err := n.constructNodeServerInfoResponse(snap, reply); err != nil {
		n.srv.logger.Printf("[ERR] server.client: failed to populate NodeUpdateResponse: %v", err)
		return err
	}
...
func (n *Node) Deregister(args *models.NodeDeregisterRequest, reply *models.NodeUpdateResponse) error {
	...
	if err != nil {
		n.srv.logger.Printf("[ERR] server.client: Deregister failed: %v", err)
		return err
	}
...
func (n *Node) UpdateStatus(args *models.NodeUpdateStatusRequest, reply *models.NodeUpdateResponse) error {
	...
	if node.Status != args.Status {
		...
		if err != nil {
			n.srv.logger.Printf("[ERR] server.client: status update failed: %v", err)
			return err
		}
		reply.NodeModifyIndex = index
	}
...
func (n *Node) batchUpdate(future *batchFuture, updates []*models.Allocation) {
	...
	if err != nil {
		n.srv.logger.Printf("[ERR] server.client: alloc update failed: %v", err)
