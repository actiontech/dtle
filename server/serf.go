...
func (s *Server) serfEventHandler() {
	for {
		...
		case e := <-s.eventCh:
			...
			default:
				s.logger.Printf("[WARN] udup: unhandled serf event: %#v", e)
			}
...
func (s *Server) nodeJoin(me serf.MemberEvent) {
	for _, m := range me.Members {
		...
		if !ok {
			s.logger.Printf("[WARN] udup: non-server in gossip pool: %s", m.Name)
			continue
		}
		s.logger.Printf("[INFO] udup: adding server %s", parts)
...
func (s *Server) maybeBootstrap() {
	...
	if err != nil {
		s.logger.Printf("[ERR] udup: failed to read last raft index: %v", err)
		return
	}
	...
	for _, member := range members {
		...
		if p.Expect != 0 && p.Expect != int(atomic.LoadInt32(&s.config.BootstrapExpect)) {
			s.logger.Printf("[ERR] udup: peer %v has a conflicting expect value. All nodes should expect the same number.", member)
			return
		}
		if p.Bootstrap {
			s.logger.Printf("[ERR] udup: peer %v has bootstrap mode. Expect disabled.", member)
			return
		}
	...
	// Update the peer set
	s.logger.Printf("[INFO] udup: Attempting bootstrap with nodes: %v", addrs)
	if err := s.raft.SetPeers(addrs).Error(); err != nil {
		s.logger.Printf("[ERR] udup: failed to bootstrap peers: %v", err)
	}
...
func (s *Server) nodeFailed(me serf.MemberEvent) {
	for _, m := range me.Members {
		...
		s.logger.Printf("[INFO] udup: removing server %s", parts)
