...
func (p *PeriodicDispatch) Add(job *structs.Job) error {
	...
	if tracked {
		...
		p.logger.Printf("[DEBUG] udup.periodic: updated periodic job %q", job.ID)
	} else {
		if err := p.heap.Push(job, next); err != nil {
			return fmt.Errorf("failed to add job %v: %v", job.ID, err)
		}
		p.logger.Printf("[DEBUG] udup.periodic: registered periodic job %q", job.ID)
	}
...
func (p *PeriodicDispatch) removeLocked(jobID string) error {
	...
	p.logger.Printf("[DEBUG] udup.periodic: deregistered periodic job %q", jobID)
	return nil
}
...
func (p *PeriodicDispatch) run() {
	...
	for p.shouldRun() {
		...
		if launch.IsZero() {
			...
			p.logger.Printf("[DEBUG] udup.periodic: launching job %q in %s", job.ID, launchDur)
		}
...
func (p *PeriodicDispatch) dispatch(job *structs.Job, launchTime time.Time) {
	...
	if err := p.heap.Update(job, nextLaunch); err != nil {
		p.logger.Printf("[ERR] udup.periodic: failed to update next launch of periodic job %q: %v", job.ID, err)
	}
	...
	if job.Periodic.ProhibitOverlap {
		...
		if err != nil {
			msg := fmt.Sprintf("[ERR] udup.periodic: failed to determine if"+
		...
		if running {
			msg := fmt.Sprintf("[DEBUG] udup.periodic: skipping launch of"+
	...
	p.logger.Printf("[DEBUG] udup.periodic: launching job %v at %v", job.ID, launchTime)
	p.l.Unlock()
...
func (p *PeriodicDispatch) createEval(periodicJob *structs.Job, time time.Time) (*structs.Evaluation, error) {
	...
	if err != nil {
		p.logger.Printf("[ERR] udup.periodic: failed to dispatch job %q: %v", periodicJob.ID, err)
		return nil, err
	}
...
func (p *PeriodicDispatch) deriveJob(periodicJob *structs.Job, time time.Time) (
	...
	defer func() {
		if r := recover(); r != nil {
			p.logger.Printf("[ERR] udup.periodic: deriving job from"+
