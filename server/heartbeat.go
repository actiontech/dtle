...
func (s *Server) invalidateHeartbeat(id string) {
	defer metrics.MeasureSince([]string{"udup", "heartbeat", "invalidate"}, time.Now())
	// Clear the heartbeat timer
	s.heartbeatTimersLock.Lock()
	delete(s.heartbeatTimers, id)
	s.heartbeatTimersLock.Unlock()
	s.logger.Printf("[WARN] udup.heartbeat: node '%s' TTL expired", id)
	...
	if err := s.endpoints.Node.UpdateStatus(&req, &resp); err != nil {
		s.logger.Printf("[ERR] udup.heartbeat: update status failed: %v", err)
	}
}
...
func (s *Server) heartbeatStats() {
	for {
		...
		case <-time.After(5 * time.Second):
			...
			s.heartbeatTimersLock.Unlock()
			metrics.SetGauge([]string{"udup", "heartbeat", "active"}, float32(num))
