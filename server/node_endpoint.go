...
func (n *Node) Register(args *structs.NodeRegisterRequest, reply *structs.NodeUpdateResponse) error {
	...
	defer metrics.MeasureSince([]string{"udup", "client", "register"}, time.Now())
	...
	if err != nil {
		n.srv.logger.Printf("[ERR] udup.client: Register failed: %v", err)
		return err
	}
	...
	if structs.ShouldDrainNode(args.Node.Status) || transitionToReady {
		...
		if err != nil {
			n.srv.logger.Printf("[ERR] udup.client: eval creation failed: %v", err)
			return err
		}
	...
	if !args.Node.TerminalStatus() {
		...
		if err != nil {
			n.srv.logger.Printf("[ERR] udup.client: heartbeat reset failed: %v", err)
			return err
		}
	...
	if err := n.constructNodeServerInfoResponse(snap, reply); err != nil {
		n.srv.logger.Printf("[ERR] udup.client: failed to populate NodeUpdateResponse: %v", err)
		return err
	}
...
func (n *Node) Deregister(args *structs.NodeDeregisterRequest, reply *structs.NodeUpdateResponse) error {
	...
	defer metrics.MeasureSince([]string{"udup", "client", "deregister"}, time.Now())
	...
	if err != nil {
		n.srv.logger.Printf("[ERR] udup.client: Deregister failed: %v", err)
		return err
	}
...
func (n *Node) UpdateStatus(args *structs.NodeUpdateStatusRequest, reply *structs.NodeUpdateResponse) error {
	...
	defer metrics.MeasureSince([]string{"udup", "client", "update_status"}, time.Now())
	...
	if node.Status != args.Status {
		...
		if err != nil {
			n.srv.logger.Printf("[ERR] udup.client: status update failed: %v", err)
			return err
		}
		reply.NodeModifyIndex = index
	}
...
func (n *Node) UpdateDrain(args *structs.NodeUpdateDrainRequest,
	...
	defer metrics.MeasureSince([]string{"udup", "client", "update_drain"}, time.Now())
	...
	if node.Drain != args.Drain {
		...
		if err != nil {
			n.srv.logger.Printf("[ERR] udup.client: drain update failed: %v", err)
			return err
		}
		reply.NodeModifyIndex = index
	}
...
func (n *Node) Evaluate(args *structs.NodeEvaluateRequest, reply *structs.NodeUpdateResponse) error {
	...
	defer metrics.MeasureSince([]string{"udup", "client", "evaluate"}, time.Now())
...
func (n *Node) GetNode(args *structs.NodeSpecificRequest,
	...
	defer metrics.MeasureSince([]string{"udup", "client", "get_node"}, time.Now())
...
func (n *Node) GetAllocs(args *structs.NodeSpecificRequest,
	...
	defer metrics.MeasureSince([]string{"udup", "client", "get_allocs"}, time.Now())
...
func (n *Node) GetClientAllocs(args *structs.NodeSpecificRequest,
	...
	defer metrics.MeasureSince([]string{"udup", "client", "get_client_allocs"}, time.Now())
...
func (n *Node) UpdateAlloc(args *structs.AllocUpdateRequest, reply *structs.GenericResponse) error {
	...
	defer metrics.MeasureSince([]string{"udup", "client", "update_alloc"}, time.Now())
...
func (n *Node) batchUpdate(future *batchFuture, updates []*structs.Allocation) {
	...
	if err != nil {
		n.srv.logger.Printf("[ERR] udup.client: alloc update failed: %v", err)
...
func (n *Node) List(args *structs.NodeListRequest,
	...
	defer metrics.MeasureSince([]string{"udup", "client", "list"}, time.Now())
