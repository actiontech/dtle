	...
	for {
		...
		case isLeader := <-s.leaderCh:
			if isLeader {
				...
				s.logger.Printf("[INFO] udup: cluster leadership acquired")
			} else if stopCh != nil {
				close(stopCh)
				stopCh = nil
				s.logger.Printf("[INFO] udup: cluster leadership lost")
			}
		case <-s.shutdownCh:
			return
		}
...
RECONCILE:
	...
	if err := barrier.Error(); err != nil {
		s.logger.Printf("[ERR] udup: failed to wait for barrier: %v", err)
		goto WAIT
	}
	metrics.MeasureSince([]string{"udup", "leader", "barrier"}, start)

	// Check if we need to handle initial leadership actions
	if !establishedLeader {
		if err := s.establishLeadership(stopCh); err != nil {
			s.logger.Printf("[ERR] udup: failed to establish leadership: %v",
				err)
			goto WAIT
		}
	...
	if err := s.reconcile(); err != nil {
		s.logger.Printf("[ERR] udup: failed to reconcile: %v", err)
		goto WAIT
	}
...
func (s *Server) establishLeadership(stopCh chan struct{}) error {
	...
	if err := s.initializeHeartbeatTimers(); err != nil {
		s.logger.Printf("[ERR] udup: heartbeat timer setup failed: %v", err)
		return err
	}
...
func (s *Server) restorePeriodicDispatcher() error {
	...
	for i := iter.Next(); i != nil; i = iter.Next() {
		...
		if _, err := s.periodicDispatcher.ForceRun(job.ID); err != nil {
			...
			s.logger.Printf("[ERR] udup.periodic: %s", msg)
			return errors.New(msg)
		}
		s.logger.Printf("[DEBUG] udup.periodic: periodic job %q force"+
...
func (s *Server) schedulePeriodic(stopCh chan struct{}) {
	...
	getLatest := func() (uint64, bool) {
		...
		if err != nil {
			s.logger.Printf("[ERR] udup: failed to determine state store's index: %v", err)
			return 0, false
		}
...
func (s *Server) reapFailedEvaluations(stopCh chan struct{}) {
	for {
		...
		default:
			...
			s.logger.Printf("[WARN] udup: eval %#v reached delivery limit, marking as failed", newEval)
			...
			if _, _, err := s.raftApply(structs.EvalUpdateRequestType, &req); err != nil {
				s.logger.Printf("[ERR] udup: failed to update failed eval %#v: %v", newEval, err)
				continue
			}
...
func (s *Server) reapDupBlockedEvaluations(stopCh chan struct{}) {
	for {
		...
		default:
			...
			if _, _, err := s.raftApply(structs.EvalUpdateRequestType, &req); err != nil {
				s.logger.Printf("[ERR] udup: failed to update duplicate evals %#v: %v", cancel, err)
				continue
			}
		}
	}
...
func (s *Server) revokeLeadership() error {
	...
	if err := s.clearAllHeartbeatTimers(); err != nil {
		s.logger.Printf("[ERR] udup: clearing heartbeat timers failed: %v", err)
		return err
	}
...
func (s *Server) reconcile() error {
	defer metrics.MeasureSince([]string{"udup", "leader", "reconcile"}, time.Now())
...
func (s *Server) reconcileMember(member serf.Member) error {
	...
	defer metrics.MeasureSince([]string{"udup", "leader", "reconcileMember"}, time.Now())
	...
	if err != nil {
		s.logger.Printf("[ERR] udup: failed to reconcile member: %v: %v",
			member, err)
		return err
	}
	return nil
...
func (s *Server) addRaftPeer(m serf.Member, parts *serverParts) error {
	...
	if parts.Bootstrap {
		...
		for _, member := range members {
			...
			if valid && member.Name != m.Name && p.Bootstrap {
				s.logger.Printf("[ERR] udup: '%v' and '%v' are both in bootstrap mode. Only one node should be in bootstrap mode, not adding Raft peer.", m.Name, member.Name)
				return nil
			}
		}
	}
	...
	if err := future.Error(); err != nil && err != raft.ErrKnownPeer {
		s.logger.Printf("[ERR] udup: failed to add raft peer: %v", err)
		return err
	} else if err == nil {
		s.logger.Printf("[INFO] udup: added raft peer: %v", parts)
	}
	return nil
}
...
func (s *Server) removeRaftPeer(m serf.Member, parts *serverParts) error {
	...
	if err := future.Error(); err != nil && err != raft.ErrUnknownPeer {
		s.logger.Printf("[ERR] udup: failed to remove raft peer '%v': %v",
			parts, err)
		return err
	} else if err == nil {
		s.logger.Printf("[INFO] udup: removed server '%s' as peer", m.Name)
	}
	return nil
}
