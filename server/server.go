...
const (
	// datacenterQueryLimit sets the max number of DCs that a Udup
	// Server will query to find bootstrap_expect servers.
	datacenterQueryLimit = 25

	// maxStaleLeadership is the maximum time we will permit this Udup
	...
	// to query Consul for Udup Servers.
...
func NewServer(config *Config, logger *log.Logger) (*Server, error) {
	...
	if err := s.setupRPC(); err != nil {
		s.Shutdown()
		s.logger.Printf("[ERR] udup: failed to start RPC layer: %s", err)
	...
	if err := s.setupRaft(); err != nil {
		s.Shutdown()
		s.logger.Printf("[ERR] udup: failed to start Raft: %s", err)
	...
	if err != nil {
		s.Shutdown()
		s.logger.Printf("[ERR] udup: failed to start serf WAN: %s", err)
	...
	if err := s.setupWorkers(); err != nil {
		s.Shutdown()
		s.logger.Printf("[ERR] udup: failed to start workers: %s", err)
...
func (s *Server) Shutdown() error {
	s.logger.Printf("[INFO] udup: shutting down server")
	s.shutdownLock.Lock()
	...
	if s.raft != nil {
		...
		if err := future.Error(); err != nil {
			s.logger.Printf("[WARN] udup: Error shutting down raft: %s", err)
		}
		if s.raftStore != nil {
			s.raftStore.Close()
		}
...
func (s *Server) Leave() error {
	...
	if err != nil {
		s.logger.Printf("[ERR] udup: failed to check raft peers: %v", err)
		return err
	}
	...
	if isLeader && numPeers > 0 {
		...
		if err := future.Error(); err != nil && err != raft.ErrUnknownPeer {
			s.logger.Printf("[ERR] udup: failed to remove ourself as raft peer: %v", err)
		}
	}
	...
	if s.serf != nil {
		if err := s.serf.Leave(); err != nil {
			s.logger.Printf("[ERR] udup: failed to leave Serf cluster: %v", err)
		}
	}
	...
	if !isLeader {
		...
		if numPeers != 0 {
			s.logger.Printf("[WARN] udup: failed to leave raft peer set gracefully, timeout")
		}
	}
	return nil
}
...
func (s *Server) setupWorkers() error {
	...
	if len(s.config.EnabledSchedulers) == 0 || s.config.NumSchedulers == 0 {
		s.logger.Printf("[WARN] udup: no enabled schedulers")
		return nil
	}
	...
	s.logger.Printf("[INFO] udup: starting %d scheduling worker(s) for %v",
