...
func (b *EvalBroker) EmitStats(period time.Duration, stopCh chan struct{}) {
	for {
		...
		case <-time.After(period):
			stats := b.Stats()
			metrics.SetGauge([]string{"udup", "broker", "total_ready"}, float32(stats.TotalReady))
			metrics.SetGauge([]string{"udup", "broker", "total_unacked"}, float32(stats.TotalUnacked))
			metrics.SetGauge([]string{"udup", "broker", "total_blocked"}, float32(stats.TotalBlocked))
			metrics.SetGauge([]string{"udup", "broker", "total_waiting"}, float32(stats.TotalWaiting))
			for sched, schedStats := range stats.ByScheduler {
				metrics.SetGauge([]string{"udup", "broker", sched, "ready"}, float32(schedStats.Ready))
				metrics.SetGauge([]string{"udup", "broker", sched, "unacked"}, float32(schedStats.Unacked))
			}
