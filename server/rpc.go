...
func (s *Server) listen() {
	for {
		...
		if err != nil {
			...
			s.logger.Printf("[ERR] udup.rpc: failed to accept RPC conn: %v", err)
			continue
		}

		go s.handleConn(conn)
		metrics.IncrCounter([]string{"udup", "rpc", "accept_conn"}, 1)
	}
}
...
func (s *Server) handleConn(conn net.Conn) {
	...
	if _, err := conn.Read(buf); err != nil {
		if err != io.EOF {
			s.logger.Printf("[ERR] udup.rpc: failed to read byte: %v", err)
		}
		conn.Close()
		return
	}
	...
	case rpcRaft:
		metrics.IncrCounter([]string{"udup", "rpc", "raft_handoff"}, 1)
	...
	default:
		s.logger.Printf("[ERR] udup.rpc: unrecognized RPC byte: %v", buf[0])
		conn.Close()
		return
	}
}
...
func (s *Server) handleMultiplex(conn net.Conn) {
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				s.logger.Printf("[ERR] udup.rpc: multiplex conn accept failed: %v", err)
			}
			return
		}
	...
	for {
		...
		if err := s.rpcServer.ServeRequest(rpcCodec); err != nil {
			if err != io.EOF && !strings.Contains(err.Error(), "closed") {
				s.logger.Printf("[ERR] udup.rpc: RPC error: %v (%v)", err, conn)
				metrics.IncrCounter([]string{"udup", "rpc", "request_error"}, 1)
			}
			return
		}
		metrics.IncrCounter([]string{"udup", "rpc", "request"}, 1)
	}
}
...
func (s *Server) forwardRegion(region, method string, args interface{}, reply interface{}) error {
	...
	if len(servers) == 0 {
		s.peerLock.RUnlock()
		s.logger.Printf("[WARN] udup.rpc: RPC request for region '%s', no path found",
			region)
	...
	metrics.IncrCounter([]string{"udup", "rpc", "cross-region", region}, 1)
...
func (s *Server) raftApplyFuture(t structs.MessageType, msg interface{}) (raft.ApplyFuture, error) {
	...
	if n := len(buf); n > raftWarnSize {
		s.logger.Printf("[WARN] udup: Attempting to apply large raft entry (type %d) (%d bytes)", t, n)
	}
...
RUN_QUERY:
	...
	metrics.IncrCounter([]string{"udup", "rpc", "query"}, 1)
	err := opts.run()
