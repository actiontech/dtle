...
func (n *udupFSM) State() *state.StateStore {
	return n.state
}
...
func (n *udupFSM) Apply(log *raft.Log) interface{} {
	...
	default:
		if ignoreUnknown {
			n.logger.Printf("[WARN] udup.fsm: ignoring unknown message type (%d), upgrade to newer version", msgType)
			return nil
		} else {
...
func (n *udupFSM) applyUpsertNode(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "register_node"}, time.Now())
	...
	if err := n.state.UpsertNode(index, req.Node); err != nil {
		n.logger.Printf("[ERR] udup.fsm: UpsertNode failed: %v", err)
		return err
	}
...
func (n *udupFSM) applyDeregisterNode(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "deregister_node"}, time.Now())
	...
	if err := n.state.DeleteNode(index, req.NodeID); err != nil {
		n.logger.Printf("[ERR] udup.fsm: DeleteNode failed: %v", err)
		return err
	}
	return nil
}
...
func (n *udupFSM) applyStatusUpdate(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "node_status_update"}, time.Now())
	...
	if err := n.state.UpdateNodeStatus(index, req.NodeID, req.Status); err != nil {
		n.logger.Printf("[ERR] udup.fsm: UpdateNodeStatus failed: %v", err)
		return err
	}
	...
	if req.Status == structs.NodeStatusReady {
		...
		if err != nil {
			n.logger.Printf("[ERR] udup.fsm: looking up node %q failed: %v", req.NodeID, err)
			return err
...
func (n *udupFSM) applyDrainUpdate(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "node_drain_update"}, time.Now())
	...
	if err := n.state.UpdateNodeDrain(index, req.NodeID, req.Drain); err != nil {
		n.logger.Printf("[ERR] udup.fsm: UpdateNodeDrain failed: %v", err)
		return err
	}
	return nil
}
...
func (n *udupFSM) applyUpsertJob(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "register_job"}, time.Now())
	...
	// reflect.DeepEqual. Starting Udup 0.4.1, job submission sanatizes
	// the incoming job.
	req.Job.Canonicalize()

	if err := n.state.UpsertJob(index, req.Job); err != nil {
		n.logger.Printf("[ERR] udup.fsm: UpsertJob failed: %v", err)
		return err
	}
	...
	if err := n.periodicDispatcher.Add(req.Job); err != nil {
		n.logger.Printf("[ERR] udup.fsm: periodicDispatcher.Add failed: %v", err)
		return err
	}
	...
	if req.Job.IsPeriodic() {
		...
		if err != nil {
			n.logger.Printf("[ERR] udup.fsm: PeriodicLaunchByID failed: %v", err)
			return err
		}
		...
		if prevLaunch == nil {
			...
			if err := n.state.UpsertPeriodicLaunch(index, launch); err != nil {
				n.logger.Printf("[ERR] udup.fsm: UpsertPeriodicLaunch failed: %v", err)
				return err
			}
		}
	}
	...
	if parentID != "" {
		...
		if err != nil {
			n.logger.Printf("[ERR] udup.fsm: JobByID(%v) lookup for parent failed: %v", parentID, err)
			return err
		} else if parent == nil {
		...
		if parent.IsPeriodic() {
			...
			if err != nil {
				n.logger.Printf("[ERR] udup.fsm: LaunchTime(%v) failed: %v", req.Job.ID, err)
				return err
			}
...
func (n *udupFSM) applyDeregisterJob(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "deregister_job"}, time.Now())
	...
	if err := n.state.DeleteJob(index, req.JobID); err != nil {
		n.logger.Printf("[ERR] udup.fsm: DeleteJob failed: %v", err)
		return err
	}

	if err := n.periodicDispatcher.Remove(req.JobID); err != nil {
		n.logger.Printf("[ERR] udup.fsm: periodicDispatcher.Remove failed: %v", err)
		return err
	}
...
func (n *udupFSM) applyUpdateEval(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "update_eval"}, time.Now())
	...
	if err := n.state.UpsertEvals(index, req.Evals); err != nil {
		n.logger.Printf("[ERR] udup.fsm: UpsertEvals failed: %v", err)
		return err
	}
...
func (n *udupFSM) applyDeleteEval(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "delete_eval"}, time.Now())
	...
	if err := n.state.DeleteEval(index, req.Evals, req.Allocs); err != nil {
		n.logger.Printf("[ERR] udup.fsm: DeleteEval failed: %v", err)
		return err
	}
	return nil
}
...
func (n *udupFSM) applyAllocUpdate(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "alloc_update"}, time.Now())
	...
	if err := n.state.UpsertAllocs(index, req.Alloc); err != nil {
		n.logger.Printf("[ERR] udup.fsm: UpsertAllocs failed: %v", err)
		return err
	}
	return nil
}
...
func (n *udupFSM) applyAllocClientUpdate(buf []byte, index uint64) interface{} {
	defer metrics.MeasureSince([]string{"udup", "fsm", "alloc_client_update"}, time.Now())
	var req structs.AllocUpdateRequest
	if err := structs.Decode(buf, &req); err != nil {
		panic(fmt.Errorf("failed to decode request: %v", err))
	}
	...
	if err := n.state.UpdateAllocsFromClient(index, req.Alloc); err != nil {
		n.logger.Printf("[ERR] udup.fsm: UpdateAllocFromClient failed: %v", err)
		return err
	}
	...
	for _, alloc := range req.Alloc {
		if alloc.ClientStatus == structs.AllocClientStatusComplete ||
			...
			if err != nil || node == nil {
				n.logger.Printf("[ERR] udup.fsm: looking up node %q failed: %v", nodeID, err)
				return err
...
func (s *udupSnapshot) Persist(sink raft.SnapshotSink) error {
	defer metrics.MeasureSince([]string{"udup", "fsm", "persist"}, time.Now())
	// Register the nodes
...
func (s *udupSnapshot) persistPeriodicLaunches(sink raft.SnapshotSink,
	encoder *codec.Encoder) error {
	// Get all the jobs
