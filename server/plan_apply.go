...
func (s *Server) planApply() {
	...
	for {
		...
		if waitCh == nil || snap == nil {
			...
			if err != nil {
				s.logger.Printf("[ERR] udup: failed to snapshot state: %v", err)
		...
		if err != nil {
			s.logger.Printf("[ERR] udup: failed to evaluate plan: %v", err)
			pending.respond(nil, err)
			continue
		}
		...
		if err != nil {
			s.logger.Printf("[ERR] udup: failed to submit plan: %v", err)
			pending.respond(nil, err)
			continue
		}
...
func (s *Server) asyncPlanWait(waitCh chan struct{}, future raft.ApplyFuture,
	...
	if err := future.Error(); err != nil {
		s.logger.Printf("[ERR] udup: failed to apply plan: %v", err)
		pending.respond(nil, err)
		return
	}
