...
func (j *Job) Diff(other *Job, contextual bool) (*JobDiff, error) {
	...
	if j == nil && other == nil {
		...
		newPrimitiveFlat = Flatten(other, filter, true)
		diff.ID = other.ID
	} else if other == nil {
		other = &Job{}
		jUpdate = &j.Update
		diff.Type = DiffTypeDeleted
		oldPrimitiveFlat = Flatten(j, filter, true)
		diff.ID = j.ID
	} else {
		if j.ID != other.ID {
...
func (tg *TaskGroup) Diff(other *TaskGroup, contextual bool) (*TaskGroupDiff, error) {
	...
	if tg == nil && other == nil {
		...
		diff.Type = DiffTypeDeleted
		diff.Name = tg.Name
		oldPrimitiveFlat = Flatten(tg, filter, true)
	} else {
...
func (t *Task) Diff(other *Task, contextual bool) (*TaskDiff, error) {
	...
	if t == nil && other == nil {
		...
		diff.Type = DiffTypeDeleted
		diff.Name = t.Name
		oldPrimitiveFlat = Flatten(t, filter, true)
	} else {
...
func (r *Resources) Diff(other *Resources, contextual bool) *ObjectDiff {
	...
	if reflect.DeepEqual(r, other) {
		...
		r = &Resources{}
		diff.Type = DiffTypeAdded
		newPrimitiveFlat = Flatten(other, nil, true)
	} else if other == nil {
		other = &Resources{}
		diff.Type = DiffTypeDeleted
		oldPrimitiveFlat = Flatten(r, nil, true)
	} else {
		diff.Type = DiffTypeEdited
		oldPrimitiveFlat = Flatten(r, nil, true)
		newPrimitiveFlat = Flatten(other, nil, true)
...
func (r *NetworkResource) Diff(other *NetworkResource, contextual bool) *ObjectDiff {
	...
	if reflect.DeepEqual(r, other) {
		...
		diff.Type = DiffTypeDeleted
		oldPrimitiveFlat = Flatten(r, filter, true)
	} else {
		diff.Type = DiffTypeEdited
		oldPrimitiveFlat = Flatten(r, filter, true)
		newPrimitiveFlat = Flatten(other, filter, true)
...
func configDiff(old, new map[string]interface{}, contextual bool) *ObjectDiff {
	...
	// Diff the primitive fields.
	oldPrimitiveFlat := Flatten(old, nil, false)
	newPrimitiveFlat := Flatten(new, nil, false)
	diff.Fields = fieldDiffs(oldPrimitiveFlat, newPrimitiveFlat, contextual)
	return diff
}
...
func primitiveObjectDiff(old, new interface{}, filter []string, name string, contextual bool) *ObjectDiff {
	oldPrimitiveFlat := Flatten(old, filter, true)
	newPrimitiveFlat := Flatten(new, filter, true)
