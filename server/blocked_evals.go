...
func (b *BlockedEvals) EmitStats(period time.Duration, stopCh chan struct{}) {
	for {
		...
		case <-time.After(period):
			stats := b.Stats()
			metrics.SetGauge([]string{"udup", "blocked_evals", "total_blocked"}, float32(stats.TotalBlocked))
			metrics.SetGauge([]string{"udup", "blocked_evals", "total_escaped"}, float32(stats.TotalEscaped))
		case <-stopCh:
			return
		}
	}
