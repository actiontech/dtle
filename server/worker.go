...
REQ:
	...
	metrics.MeasureSince([]string{"udup", "worker", "dequeue_eval"}, start)
	if err != nil {
...
func (w *Worker) sendAck(evalID, token string, ack bool) {
	defer metrics.MeasureSince([]string{"udup", "worker", "send_ack"}, time.Now())
	// Setup the request
...
func (w *Worker) waitForIndex(index uint64, timeout time.Duration) error {
	...
	start := time.Now()
	defer metrics.MeasureSince([]string{"udup", "worker", "wait_for_index"}, start)
CHECK:
...
func (w *Worker) invokeScheduler(eval *structs.Evaluation, token string) error {
	defer metrics.MeasureSince([]string{"udup", "worker", "invoke_scheduler", eval.Type}, time.Now())
...
func (w *Worker) SubmitPlan(plan *structs.Plan) (*structs.PlanResult, scheduler.State, error) {
	...
	defer metrics.MeasureSince([]string{"udup", "worker", "submit_plan"}, time.Now())
...
func (w *Worker) UpdateEval(eval *structs.Evaluation) error {
	...
	defer metrics.MeasureSince([]string{"udup", "worker", "update_eval"}, time.Now())
...
func (w *Worker) CreateEval(eval *structs.Evaluation) error {
	...
	defer metrics.MeasureSince([]string{"udup", "worker", "create_eval"}, time.Now())
...
func (w *Worker) ReblockEval(eval *structs.Evaluation) error {
	...
	defer metrics.MeasureSince([]string{"udup", "worker", "reblock_eval"}, time.Now())
