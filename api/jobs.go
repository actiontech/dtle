...
const (
	// JobTypeSync indicates a long-running processes
	JobTypeSync = "sync"

	// JobTypeMigrate indicates a short-lived process
...
// NewSyncJob creates and returns a new service-style job
// for long-lived processes using the provided name, ID, and
// relative job priority.
func NewSyncJob(id, name, region string, pri int) *Job {
	return newJob(id, name, region, JobTypeSync, pri)
}

// NewMigrateJob creates and returns a new batch-style job for
// short-lived processes using the provided name and ID along
// with the relative job priority.
func NewMigrateJob(id, name, region string, pri int) *Job {
	return newJob(id, name, region, JobTypeMigrate, pri)
}
